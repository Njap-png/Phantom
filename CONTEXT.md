...
## Current State (as of latest commit: `ea7bc25`)

**100+ hacker tools** in dual runtime (`.mjs` zero-dep + `src/` TypeScript):
...
| GUI | dashboard | 1 |
| Bug bounty | hackerone (programs, scope, reports, submit) | 1 |
...
- [x] **HackerOne bug bounty integration** — programs, scope, reports, submit, test auth
- [x] **Git credentials auto-config** — Auto-push evolution improvements
- [x] **Config case-sensitivity** — Preserve token case for API credentials
...
- `HACKERONE_API_USERNAME` — HackerOne API token identifier (Basic Auth username)
- `HACKERONE_API_TOKEN` — HackerOne API token value (Basic Auth password)
...
- Auth: Token stored in config (`@config|set GIT_TOKEN <token>`)
- Latest: 4 commits ahead (HackerOne auth fix, config case-sensitivity, test auth, CONTEXT update)
