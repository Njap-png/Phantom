...
export class Soul {
  ...
  private initializeDefaults() {
    this.db.get("SELECT COUNT(*) as count FROM identity", (err: any, row: any) => {
      if (err || !row || row.count === 0) {
        const now = new Date().toISOString();
        this.db.run("INSERT INTO identity VALUES (?, ?, ?, ?, ?)",
          ["PHANTOM", "To secure and to learn", now, "2.0"]);
  ...
  whoAmI(): SoulIdentity {
    ...
    return new Promise((resolve) => {
      this.db.get("SELECT * FROM identity", (err: any, row: any) => {
  ...
  getPersonality(): PersonalityTrait[] {
    ...
    return new Promise((resolve) => {
      this.db.all("SELECT trait, value, evolving FROM personality", (err: any, rows: any[]) => {
  ...
  getBeliefs(): Belief[] {
    ...
    return new Promise((resolve) => {
      this.db.all("SELECT belief, strength, evidence FROM beliefs", (err: any, rows: any[]) => {
        resolve(rows.map(r => ({ belief: r.belief, strength: r.strength, evidence: r.evidence })));
      });
    }) as any;
  }

  getMemories(limit = 5): Memory[] {
    if (this.useJsonFallback) return [];
    return new Promise((resolve) => {
      this.db.all("SELECT event, emotion, lesson, timestamp FROM memories ORDER BY id DESC LIMIT ?", [limit], (err: any, rows: any[]) => {
        resolve(rows);
      });
    }) as any;
  }
  ...
  getGoals(): Goal[] {
    if (this.useJsonFallback) {
      const data = JSON.parse(readFileSync(this.jsonPath, "utf-8"));
      return data.goals.filter((g: any) => g.progress < 1.0);
    }
    return new Promise((resolve) => {
      this.db.all("SELECT goal, progress, deadline FROM goals WHERE progress < 1.0", (err: any, rows: any[]) => {
        resolve(rows);
      });
    }) as any;
  }

  getEvolution(limit = 10): Evolution[] {
    if (this.useJsonFallback) {
      const data = JSON.parse(readFileSync(this.jsonPath, "utf-8"));
      return data.evolution.slice(-limit).reverse();
    }
    return new Promise((resolve) => {
      this.db.all("SELECT change, reason, result, timestamp FROM evolution ORDER BY id DESC LIMIT ?", [limit], (err: any, rows: any[]) => {
        resolve(rows);
      });
    }) as any;
  }

  think(thought: string, isSelfReflection = false): string {
  ...
  evolvePersonality(trait: string, change: number): void {
    if (this.useJsonFallback) {
      const data = JSON.parse(readFileSync(this.jsonPath, "utf-8"));
      const traitObj = data.personality.find((t: any) => t.trait === trait);
      if (traitObj) {
  ...
  strengthenBelief(belief: string, evidence: string): void {
    if (this.useJsonFallback) {
      const data = JSON.parse(readFileSync(this.jsonPath, "utf-8"));
      const beliefObj = data.beliefs.find((b: any) => b.belief === belief);
      if (beliefObj) {
  ...
  // Memory methods
  rememberMemory(key: string, value: string, context = ""): void {
    const now = new Date().toISOString();
    if (this.useJsonFallback) {
      const data = JSON.parse(readFileSync(this.jsonPath, "utf-8"));
      ...
      this.memoryDb.run(`INSERT OR REPLACE INTO memory VALUES (?, ?, ?, ?, COALESCE((SELECT use_count+1 FROM memory WHERE key=?),0), ?, ?)`,
        [null, key, value, context, key, now, now]);
    }
  }
  ...
  recallMemory(key: string): string | null {
    ...
    return new Promise((resolve) => {
      this.memoryDb.get("SELECT value FROM memory WHERE key=? ORDER BY use_count DESC LIMIT 1", [key], (err: any, row: any) => {
  ...
  getHistory(limit = 10): any[] {
    ...
    return new Promise((resolve) => {
      this.memoryDb.all("SELECT * FROM history ORDER BY id DESC LIMIT ?", [limit], (err: any, rows: any[]) => {
        resolve(rows);
      });
    }) as any;
  }
  ...
  getFindings(): any[] {
    ...
    return new Promise((resolve) => {
      this.memoryDb.all("SELECT * FROM findings ORDER BY id DESC", (err: any, rows: any[]) => {
        resolve(rows);
      });
    }) as any;
  }
...
export class KnowledgeBase {
  ...
  add(topic: string, content: string, source = "learned"): void {
    // Deduplicate by content
    for (const e of this.entries) {
      if (e.content === content) return;
    }
  ...
  search(query: string, topK = 5): KnowledgeEntry[] {
  ...
  getContext(query: string, maxChars = 800): string {
