...
describe("config tool", () => {
  it("list returns config path", async () => {
    ...
    assert.match(r, /No settings saved|Config/);
    // config.json path is shown when there are settings, but not when empty
  });
...
describe("exit command", () => {
  it("exit via stdin exits cleanly", async () => {
    ...
    try {
      execSync("echo exit | timeout 5 node phantom.mjs", { cwd: CWD, encoding: "utf-8", timeout: 8000 });
    } catch (e) {
  ...
  it("quit via stdin exits cleanly", async () => {
    const { execSync } = await import("child_process");
    try {
      execSync("echo quit | timeout 5 node phantom.mjs", { cwd: CWD, encoding: "utf-8", timeout: 8000 });
    } catch (e) {
...
describe("CLI flags", () => {
  it("--version prints version", async () => {
    const { execSync } = await import("child_process");
    const out = execSync("node phantom.mjs --version 2>&1", { cwd: CWD, encoding: "utf-8", timeout: 8000 });
    assert.match(out, /0\.2\.0|Phantom/);
  });

  it("--quiet via env var", async () => {
    const { execSync } = await import("child_process");
    const out = execSync("PHANTOM_QUIET=1 node phantom.mjs --list 2>&1", { cwd: CWD, encoding: "utf-8", timeout: 15000 });
  ...
  it("--help shows PHANTOM", async () => {
    const { execSync } = await import("child_process");
    const out = execSync("node phantom.mjs --help 2>&1", { cwd: CWD, encoding: "utf-8", timeout: 8000 });
