...
import { createOpenAIProvider, createOllamaProvider } from "../providers/openai";
export class PhantomTerminal {
    ...
    buildUI() {
        ...
        this.screen.key(["escape"], () => {
            this.commandInput.readInput((_err, val) => {
                if (val)
    ...
    registerEvents() {
        this.bus.on("agent:spawned", ((data) => {
            const agent = data;
        ...
        this.bus.on("agent:speaking", ((data) => {
            const { agent, message } = data;
            this.panelManager.writeToPanel(agent.id, `{${agent.color}-fg}[${agent.name}]{/${agent.color}-fg} ${message}`);
            this.refreshTitle();
        }));
        this.bus.on("agent:thinking", ((_data) => {
            this.refreshTitle();
        }));
        this.bus.on("agent:evolving", ((data) => {
            const { agent, level } = data;
            this.panelManager.writeToPanel(agent.id, `{magenta-fg}⬆ EVOLVED to level ${level}{/magenta-fg}`);
        }));
        this.bus.on("agent:removed", ((data) => {
            const id = data;
    ...
    makeTitleContent(count, hasThinking) {
        ...
        switch (command) {
            ...
            case "ls": {
                if (panel) {
                    ...
                    agents.forEach((a) => {
                        panel.log.add(`  {${a.color}-fg}●{/${a.color}-fg} ${a.name} (${a.id}) — ${a.role} [${a.status}] ★${a.evolutionLevel}`);
                    });
                }
                break;
            }
            ...
            case "d": {
                ...
                if (starterId) {
                    ...
                    if (panel)
                        panel.log.add(`{magenta-fg}◈ Debate started: ${topic}{/magenta-fg}`);
                }
                break;
            }
            case "evolve":
