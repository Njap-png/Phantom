const PROVIDER_CONFIGS = {
    openai: {
        ...
        fmt: (o) => ({ model: o.model, messages: o.messages, temperature: 0.7, max_tokens: 16384 }),
        parse: (d) => {
    ...
    anthropic: {
        ...
        chatPath: "/messages",
        fmt: (o) => ({ model: o.model, messages: o.messages, max_tokens: 512 }),
        parse: (d) => d.content?.[0]?.text || d.content?.toString() || "...",
    ...
    groq: {
        ...
        chatPath: "/chat/completions",
        fmt: (o) => ({ model: o.model, messages: o.messages, temperature: 0.7, max_tokens: 512 }),
    ...
    ollama: {
        ...
        defaultModel: "llama3",
        chatPath: "/api/chat",
        fmt: (o) => ({ model: o.model, messages: o.messages, stream: false }),
        parse: (d) => d.message?.content?.trim() || "...",
        auth: () => ({})
    },
    opencode: {
        url: "https://opencode.ai/zen/v1",
    ...
    return {
        async chat(messages, opts = {}) {
            ...
            for (const providerName of fallbackOrder) {
                ...
                if (provider.keyEnv && !providerKey && providerName !== "ollama")
                    continue;
                try {
                    ...
                    if (provider.urlMod)
                        url = provider.urlMod(url, provider.chatPath.replace("{model}", modelToUse), providerKey);
        ...
        async transcribe(filePath) {
            ...
            if (!openaiKey)
                return "[Transcribe] Set OPENCODE_ZEN_API_KEY";
            try {
                ...
                return `[Transcribe err] ${e.message}`;
            }
        }
    };
}
