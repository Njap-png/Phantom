...
async function decode(input) {
    ...
    try {
        if (s.includes("%"))
            results.push(`url: ${decodeURIComponent(s)}`);
    }
    catch { }
    if (/^[01\s]+$/.test(s)) {
        try {
...
async function fileAnalyze(path) {
    try {
        if (!existsSync(path))
        ...
        for (let i = 0; i < buf.length; i++)
            freq.set(buf[i], (freq.get(buf[i]) || 0) + 1);
        let entropy = 0;
        ...
        if (magic.startsWith("4d5a"))
            fileType = "PE (Windows executable/DLL)";
            ...
            fileType = "ZIP/PK archive";
        else if (magic.startsWith("25504446"))
            fileType = "PDF document";
        else if (magic.startsWith("d0cf11e0a1b11ae1"))
            fileType = "OLE2 (Office doc)";
        else if (magic.startsWith("1f8b"))
            fileType = "GZIP compressed";
        else if (magic.startsWith("424d"))
            fileType = "BMP image";
        else if (magic.startsWith("47494638"))
            fileType = "GIF image";
        else if (magic.startsWith("cafebabe"))
            fileType = "Java class";
        else if (magic.startsWith("52617221"))
            fileType = "RAR archive";
        else if (buf.slice(0, 3).toString() === "#!/")
            fileType = "Script";
        const entropyLabel = entropy > 7.2
        ...
        for (const [label, promise] of checks) {
            try {
                ...
                if (label === "SOA" && val && typeof val === "object" && !Array.isArray(val)) {
                    const soa = val;
                    results.push(`  SOA: ${soa.nsname} (admin: ${soa.hostmaster})`);
                }
...
async function sandboxExec(input) {
    ...
    try {
        if (!existsSync(binaryPath))
            return `[Sandbox] File not found: ${binaryPath}`;
        // Check file type
        ...
        try {
            execSync(`chmod +x '${sandboxPath}'`, { timeout: 3000 });
        }
        catch { }
        ...
        let hasStrace = false;
        try {
            execSync("which strace", { encoding: "utf-8", timeout: 3000 });
            hasStrace = true;
        }
        catch { }
        if (hasStrace) {
            ...
            if (existsSync(straceLog)) {
                ...
                if (netCalls.length)
                    report.push(`🌐 Network: ${netCalls.length} call(s) — possible C2 or beaconing`);
                if (fileCalls.length)
                    report.push(`📁 File I/O: ${fileCalls.length} call(s)`);
                if (procCalls.length)
                    report.push(`🔧 Process: ${procCalls.length} fork/execve(s)`);
                if (ipcCalls.length)
                    report.push(`🔗 IPC: ${ipcCalls.length} call(s)`);
            ...
            try {
                ...
                if (err.trim())
                    report.push(`📤 stderr:\n${err}`);
                if (e.status !== null)
                    report.push(`🚦 Exit code: ${e.status}`);
            }
        }
        // Cleanup on success
        report.push(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
...
// ── PE ANALYZER ────────────────────────────────────────────
async function peAnalyze(path) {
    try {
        if (!existsSync(path))
            return `[PE] File not found: ${path}`;
        const buf = readFileSync(path);
        const magic = buf.slice(0, 2).toString();
        if (magic !== "MZ")
            return `[PE] Not a PE file (no MZ magic): ${path}`;
        const lines = [`📦 PE Analysis: ${path}`, `Size: ${buf.length} bytes`, ``];
        // DOS Header
        ...
        if (peSig !== "PE\0\0")
            return `[PE] Invalid PE signature at offset 0x${e_lfanew.toString(16)}: ${peSig}`;
        ...
        if (optMagic === 0x10b || optMagic === 0x20b) {
            ...
            const subsystems = { 1: "NATIVE", 2: "WINDOWS_GUI", 3: "WINDOWS_CUI", 5: "OS2", 7: "POSIX" };
            ...
            if (dllChars & 0x40)
                lines.push(`  ⚠ DLL has TLS callbacks (malware indicator)`);
            // ── Section Table ──
            ...
            for (let i = 0; i < sections; i++) {
                ...
                if (chars & 0x20)
                    charFlags.push("CODE");
                if (chars & 0x40)
                    charFlags.push("INIT_DATA");
                if (chars & 0x80)
                    charFlags.push("UNINIT_DATA");
                if (chars & 0x20000000)
                    charFlags.push("EXECUTE");
                if (chars & 0x40000000)
                    charFlags.push("READ");
                if (chars & 0x80000000)
                    charFlags.push("WRITE");
            ...
            if (numDataDirs > 0) {
                ...
                for (let i = 0; i < Math.min(8, numDataDirs); i++) {
                    ...
                    if (va || sz)
                        lines.push(`  ${dirNames[i]?.padEnd(12)} VA: 0x${va.toString(16).padStart(8, "0")}  Size: ${sz}`);
                }
            }
        ...
        if (packerMatches.length)
            lines.push(`⚠ Packer detected: ${packerMatches.join(", ")}`);
        return lines.join("\n");
    }
    catch (e) {
...
// ── ELF ANALYZER ───────────────────────────────────────────
async function elfAnalyze(path) {
    try {
        if (!existsSync(path))
            return `[ELF] File not found: ${path}`;
        const buf = readFileSync(path);
        ...
        const osabi = buf[7];
        const osabiNames = { 0: "UNIX System V", 3: "GNU/Linux", 9: "FreeBSD", 12: "OpenBSD" };
        const etype = is64 ? buf.readUInt16LE(16) : buf.readUInt16LE(16);
        const typeNames = { 0: "NONE", 1: "REL (Relocatable)", 2: "EXEC (Executable)", 3: "DYN (Shared Object)", 4: "CORE" };
        const emachine = is64 ? buf.readUInt16LE(18) : buf.readUInt16LE(18);
        const machineNames = { 3: "i386", 62: "x86-64", 40: "ARM", 183: "AArch64", 50: "IA-64", 20: "PowerPC", 21: "PowerPC64", 8: "MIPS", 243: "RISC-V" };
        ...
        if (phnum > 0 && phoff > 0) {
            ...
            const ptNames = { 0: "NULL", 1: "LOAD", 2: "DYNAMIC", 3: "INTERP", 4: "NOTE", 5: "SHLIB", 6: "PHDR", 7: "TLS", 0x6474e550: "GNU_EH_FRAME", 0x6474e551: "GNU_STACK", 0x6474e552: "GNU_RELRO", 0x6474e553: "GNU_PROPERTY" };
            ...
            if (dynsymOff > 0 && dynstrOff > 0) {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    if (strIdx > 0 && strIdx < buf.length - dynstrOff) {
                        let name = "";
                        for (let j = dynstrOff + strIdx; j < buf.length && buf[j] !== 0; j++)
        ...
        if (etype === 2) {
            ...
            for (let i = 0; i < phnum; i++) {
                ...
                if ((is64 ? buf.readUInt32LE(off) : buf.readUInt32LE(off)) === 0x6474e551) { // GNU_STACK
                    ...
                    if (flags & 1)
                        lines.push("⚠ W+X Stack (GNU_STACK executable) — suspicious");
                }
            }
        }
        return lines.join("\n");
...
async function macroScan(path) {
    try {
        if (!existsSync(path))
            return `[MacroScan] File not found: ${path}`;
        const buf = readFileSync(path);
        ...
        if (isOLE2) {
            ...
            if (cur.length >= 4) {
                ...
                if (lower.includes("vba") || lower.includes("macro") || lower.includes("auto_"))
                    vbaStrings.push(cur.substring(0, 200));
                if (lower.includes("shell(") || lower.includes("createobject"))
                    suspiciousPatterns.push(cur.substring(0, 200));
            }
...
async function stringsDeep(path) {
    try {
        if (!existsSync(path))
            return `[StringsDeep] File not found: ${path}`;
        const buf = readFileSync(path);
        const size = buf.length;
        ...
        for (const s of allStrings) {
            ...
            if (trimmed.length > 200) {
                others.push(trimmed.substring(0, 200) + "...");
                continue;
            }
            if (trimmed.startsWith("http") && urlRe.test(trimmed)) {
                try {
                    urls.add(new URL(trimmed).href);
                }
                catch {
                    urls.add(trimmed);
                }
                ...
                if (!d.endsWith(".local") && !d.endsWith(".lan") && !d.endsWith(".localhost"))
                    domains.add(d);
            }
            else if (filepathRe.test(trimmed))
                paths.add(trimmed);
            else if (cryptoKeyRe.test(trimmed))
                cryptos.push(trimmed.substring(0, 150));
            else if (trimmed.length >= 8 && trimmed.length <= 64)
                others.push(trimmed);
        }
        const lines = [
        ...
        if (urls.size)
            lines.push(`🌐 URLs (${urls.size}):`, ...Array.from(urls).slice(0, 15).map(u => `  ${u}`), "");
        if (emails.size)
            lines.push(`📧 Emails (${emails.size}):`, ...Array.from(emails).slice(0, 10).map(e => `  ${e}`), "");
        if (ips.size)
            lines.push(`🌍 IP Addresses (${ips.size}):`, ...Array.from(ips).slice(0, 15).map(i => `  ${i}`), "");
        if (domains.size)
            lines.push(`🌐 Domains (${domains.size}):`, ...Array.from(domains).slice(0, 15).map(d => `  ${d}`), "");
        if (paths.size)
            lines.push(`📁 File Paths (${paths.size}):`, ...Array.from(paths).slice(0, 10).map(p => `  ${p}`), "");
        if (cryptos.length)
            lines.push(`🔑 Potential Secrets (${cryptos.length}):`, ...cryptos.slice(0, 10).map(c => `  ⚠ ${c}`), "");
        // Entropy
        let freq = new Map();
        for (let i = 0; i < buf.length; i++)
            freq.set(buf[i], (freq.get(buf[i]) || 0) + 1);
...
async function hexDump(input) {
    try {
        ...
        if (!existsSync(path))
            return `[HexDump] File not found: ${path}`;
        const buf = readFileSync(path);
        const size = buf.length;
        const lines = [
            `⎔ Hex Dump: ${path}`,
...
// ── IOC EXTRACTOR ──────────────────────────────────────────
async function extractIoc(input) {
    try {
        const parts = input.split("|").map(s => s.trim());
        ...
        results["Domains"] = results["Domains"].filter(d => !ipSet.has(d) && d.includes(".") && !d.match(/^\d/) && d.length > 3);
        ...
        for (const [type, iocs] of Object.entries(results)) {
            if (iocs.length) {
                ...
                if (iocs.length > 20)
                    lines.push(`  ... and ${iocs.length - 20} more`);
                lines.push(``);
            }
        }
        // Dedup all IOCs for summary
        if (totalIocs === 0)
            lines.push("(no IOCs found)");
        return lines.join("\n");
    }
    catch (e) {
...
// ── PROCESS FORENSICS ─────────────────────────────────────
async function procAnalyze(input) {
    try {
        const pid = parseInt(input.trim());
        if (isNaN(pid))
            return `[ProcAnalyze] Usage: proc_analyze|PID\nExample: @proc_analyze|1234`;
        const procDir = `/proc/${pid}`;
        if (!existsSync(procDir))
            return `[ProcAnalyze] Process ${pid} not found`;
        const lines = [
        ...
        try {
            ...
            if (cmdline.trim())
                lines.push(`  Cmdline: ${cmdline.substring(0, 500)}`);
        }
        catch { }
        // ── CWD ──
        try {
        ...
        try {
            ...
            const sensitive = envLines.filter(e => /key|secret|token|pass|cert|credential|api_key|auth/i.test(e));
        ...
        try {
            ...
            for (const fd of fds) {
                try {
                    ...
                    if (link.includes("socket:"))
                        sockets.push(link);
                    else if (link.includes("pipe:"))
                        pipes.push(link);
                    else if (link)
                        regFiles.push(link);
                }
            ...
            if (sockets.length)
                lines.push(...sockets.slice(0, 10).map(s => `    🔌 ${s}`));
            if (regFiles.length)
                lines.push(...regFiles.slice(0, 8).map(f => `    📄 ${f}`));
        }
        catch { }
        // ── Network Connections ──
        try {
                    ...
                    if (conns.length) {
                        ...
                        for (const conn of conns.slice(0, 8)) {
                            ...
                            if (parts2.length >= 4) {
                                ...
                                const stateNames = { "01": "ESTAB", "02": "SYN_SENT", "03": "SYN_RECV", "04": "FIN_WAIT", "05": "TIMEWAIT", "06": "CLOSE", "07": "CLOSE_WAIT", "0A": "LISTEN", "0B": "CLOSING" };
...
// ── WEB CLICKERS ──────────────────────────────────────────
async function webClick(input) {
    try {
        const parts = input.split("|").map(s => s.trim());
        const url = parts[0];
        if (!url)
            return `[Web Click] Usage: url|selector|method. method: index (click Nth link), text (by link text), selector (URL)`;
        const r = await fetch(url, { signal: AbortSignal.timeout(15000) });
        const html = await r.text();
        const title = (html.match(/<title[^>]*>([^<]+)<\/title>/i) || [])[1] || "";
        const links = [...html.matchAll(/<a[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)];
        const clean = links.map(([, h, t]) => [h, t.replace(/<[^>]+>/g, "").trim()]).filter(([h]) => h && !h.startsWith("#") && !h.startsWith("javascript:"));
        const selector = parts[1] || "0";
        const method = parts[2] || "index";
        if (method === "index") {
            const idx = parseInt(selector) || 0;
            if (idx < 0 || idx >= clean.length)
                return `[Web Click] Index ${idx} out of range (0-${clean.length - 1})`;
            const [href, text] = clean[idx];
            ...
            if (!m.length)
                return `[Web Click] No links with text: "${selector}"`;
            const [href, text] = m[0];
            const fullUrl = href.startsWith("http") ? href : new URL(href, url).href;
            const r2 = await fetch(fullUrl, { signal: AbortSignal.timeout(15000) });
            const h2 = await r2.text();
            ...
            const r2 = await fetch(selector, { signal: AbortSignal.timeout(15000) });
            const h2 = await r2.text();
            const t2 = (h2.match(/<title[^>]*>([^<]+)<\/title>/i) || [])[1] || "";
            const body = h2.replace(/<script[^>]*>[\s\S]*?<\/script>/gi, "").replace(/<style[^>]*>[\s\S]*?<\/style>/gi, "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
        ...
        return `[Web Click Error] ${e.message}`;
    }
}
async function webLinks(url) {
    try {
        if (!url)
            return `[Web Links] Usage: url`;
        const r = await fetch(url, { signal: AbortSignal.timeout(15000) });
        const html = await r.text();
        ...
        return `[Web Links Error] ${e.message}`;
    }
}
async function webForm(input) {
    try {
        const parts = input.split("|").map(s => s.trim());
        const url = parts[0];
        if (!url)
            return `[Web Form] Usage: url|field1=val1`;
        const r = await fetch(url, { signal: AbortSignal.timeout(15000) });
        const html = await r.text();
        ...
        for (const [action, method, inner] of forms) {
            ...
            fields.forEach(([, n, v]) => res.push(`  ${n}=${v || ""}`));
            const userFields = parts.slice(1).filter(p => p.includes("="));
            if (userFields.length > 0) {
                const params = new URLSearchParams();
                userFields.forEach(p => { const [k, v] = p.split("=", 2); if (k)
                    params.set(k, v || ""); });
        ...
        return `[Web Form Error] ${e.message}`;
    }
}
async function webSnapshot(url) {
    try {
        if (!url)
            return `[Web Snapshot] Usage: url`;
        const r = await fetch(url, { signal: AbortSignal.timeout(15000) });
        const html = await r.text();
        const title = (html.match(/<title[^>]*>([^<]+)<\/title>/i) || [])[1] || "";
        ...
        return `[Web Snapshot Error] ${e.message}`;
    }
}
// ── WORKSPACE (Project Management) ─────────────────────────
async function projectCreate(name) {
    if (!name)
        return `[Project] Usage: project_create <name>`;
    try {
        ...
        return `[Project Error] ${e.message}`;
    }
}
...
async function projectInfo(name) {
    try {
        ...
        if (!name)
            return `[Project] Usage: project_info <name>`;
        const dir = `${process.env.HOME || "/root"}/.config/phantom/projects/${name.replace(/[^a-z0-9_-]/gi, "_")}`;
        if (!existsSync(dir))
        ...
        if (meta.notes?.length)
            meta.notes.slice(-5).forEach((n, i) => lines.push(`  Note ${i + 1}: ${n.substring(0, 120)}`));
        return lines.join("\n");
    }
    catch (e) {
        return `[Project Error] ${e.message}`;
...
async function projectFileAdd(input) {
    try {
        ...
        if (!name || !filePath)
            return `[Project File] Usage: project_file_add <project>|<filepath>`;
        const dir = `${process.env.HOME || "/root"}/.config/phantom/projects/${name.replace(/[^a-z0-9_-]/gi, "_")}`;
        if (!existsSync(dir))
            return `[Project File] Not found: "${name}"`;
        const meta = JSON.parse(readFileSync(`${dir}/project.json`, "utf-8"));
        if (!existsSync(`${dir}/files`))
            mkdirSync(`${dir}/files`, { recursive: true });
        const base = filePath.split("/").pop() || filePath;
        copyFileSync(filePath, `${dir}/files/${base}`);
        meta.files = meta.files || [];
        meta.files.push({ name: base, source: filePath, added: new Date().toISOString() });
        meta.updated = new Date().toISOString();
        writeFileSync(`${dir}/project.json`, JSON.stringify(meta, null, 2));
        return `✅ Added ${base} to "${name}".`;
    }
    catch (e) {
        return `[Project File Error] ${e.message}`;
    }
}
async function projectNote(input) {
    try {
        ...
        if (!name)
            return `[Project Note] Usage: project_note <project>|<text>`;
        const dir = `${process.env.HOME || "/root"}/.config/phantom/projects/${name.replace(/[^a-z0-9_-]/gi, "_")}`;
        if (!existsSync(dir))
            return `[Project Note] Not found: "${name}"`;
        const meta = JSON.parse(readFileSync(`${dir}/project.json`, "utf-8"));
        if (!note) {
            return `📝 Notes: ${(meta.notes || []).map((n, i) => `\n  ${i + 1}. ${n.substring(0, 200)}`).join("") || "(none)"}`;
        }
        meta.notes = meta.notes || [];
        meta.notes.push(`[${new Date().toISOString().substring(0, 10)}] ${note}`);
        writeFileSync(`${dir}/project.json`, JSON.stringify(meta, null, 2));
        return `✅ Note added to "${name}".`;
    }
    catch (e) {
        return `[Project Note Error] ${e.message}`;
    }
}
async function projectSwitch(name) {
    if (!name)
        return `[Project] Usage: project_switch <name>`;
    try {
        const { existsSync, readFileSync, writeFileSync } = await import("fs");
        const dir = `${process.env.HOME || "/root"}/.config/phantom/projects/${name.replace(/[^a-z0-9_-]/gi, "_")}`;
        if (!existsSync(dir))
...
async function scope(input) {
    ...
    if (!existsSync(dir))
        mkdirSync(dir, { recursive: true });
    function load() { try {
        return existsSync(SCOPE_FILE) ? JSON.parse(readFileSync(SCOPE_FILE, "utf-8")) : [];
    }
    catch {
        return [];
    } }
    ...
    if (!input || !input.trim()) {
        const s = load();
        return s.length ? `🎯 SCOPE (${s.length})\n${s.map((t, i) => `  ${i + 1}. ${t}`).join("\n")}` : `🎯 Scope: (empty) — use "scope add <target>"`;
    }
    const parts = input.split(/\s+/).map(s => s.trim());
    const cmd = parts[0].toLowerCase();
    if (cmd === "add") {
        if (!parts[1])
            return `[Scope] Usage: scope add <target>`;
        const s = load(), t = parts.slice(1).join(" ");
        if (s.includes(t))
            return `[Scope] "${t}" already in scope.`;
        s.push(t);
        save(s);
        return `✅ Added: ${t} (${s.length} total)`;
    }
    if (cmd === "remove" || cmd === "rm") {
        if (!parts[1])
            return `[Scope] Usage: scope remove <target|#id>`;
        const s = load();
        const idx = parseInt(parts[1]);
        if (!isNaN(idx) && idx > 0 && idx <= s.length) {
            const r = s.splice(idx - 1, 1);
            save(s);
            return `❌ Removed: ${r[0]}`;
        }
    ...
    if (cmd === "check") {
        if (!parts[1])
            return `[Scope] Usage: scope check <target>`;
        const s = load(), t = parts.slice(1).join(" ").toLowerCase();
        const f = s.filter((x) => t.includes(x.toLowerCase()) || x.toLowerCase().includes(t));
        return f.length ? `✅ "${parts.slice(1).join(" ")}" matches: ${f.join(", ")}` : `⚠️ "${parts.slice(1).join(" ")}" NOT in scope`;
    }
    if (cmd === "clear") {
        save([]);
        return `🗑️ Scope cleared`;
    }
    if (cmd === "export") {
        const s = load();
...
async function katana(input) {
    if (!input || !input.trim())
        return `[katana] Usage: @katana|<url> [options]`;
    try {
        ...
        try {
            execFileSync("which", ["katana"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        if (!lines.length)
            return `[katana] 0 URLs for ${url}`;
        const unique = [...new Set(lines)];
        const result = [`🔎 Katana Crawl: ${url}`, `URLs: ${unique.length}`];
        const display = unique.slice(0, 50);
        result.push(...display.map((u) => `  ${u}`));
        if (unique.length > 50)
            result.push(`  ... and ${unique.length - 50} more`);
        return result.join("\n");
    }
    catch (e) {
        return `[katana Error] ${e.message}`;
    }
}
async function subfinder(input) {
    if (!input || !input.trim())
        return `[subfinder] Usage: @subfinder|<domain> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        const parts = input.trim().split(/\s+/);
        ...
        const extra = parts.slice(1);
        try {
            execFileSync("which", ["subfinder"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[subfinder] 0 subs for ${domain}`;
        const subs = lines.map((l) => { try {
            return JSON.parse(l).host || l;
        }
        catch {
            return l;
        } });
        ...
        if (unique.length > 100)
            result.push(`  ... and ${unique.length - 100} more`);
        return result.join("\n");
    }
    catch (e) {
        return `[subfinder Error] ${e.message}`;
    }
}
async function ffuf(input) {
    if (!input || !input.trim())
        return `[ffuf] Usage: @ffuf|-u URL -w wordlist [options]`;
    try {
        const { execFileSync } = await import("child_process");
        const parts = input.trim().split(/\s+/);
        try {
            execFileSync("which", ["ffuf"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        if (!args.includes("-o"))
            args.push("-o", "/dev/stdout", "-of", "json");
        args.push("-s");
        ...
        return `[ffuf Error] ${e.message}`;
    }
}
async function httpx(input) {
    if (!input || !input.trim())
        return `[httpx] Usage: @httpx|<domain_or_file> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["httpx"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        if (!lines.length)
            return `[httpx] No alive for ${target}`;
        return [`🔎 Httpx: ${target}`, `Alive: ${lines.length}`, ...lines.slice(0, 50).map(l => `  ${l}`), ...(lines.length > 50 ? [`  ... and ${lines.length - 50} more`] : [])].join("\n");
    }
    catch (e) {
        return `[httpx Error] ${e.message}`;
    }
}
async function nuclei(input) {
    if (!input || !input.trim())
        return `[nuclei] Usage: @nuclei|<URL> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["nuclei"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        const extra = parts.slice(1);
        try {
            execFileSync("nuclei", ["-ut", "-silent"], { encoding: "utf-8", timeout: 30000 });
        }
        catch { }
        const out = execFileSync("nuclei", ["-u", target, "-o", "/dev/stdout", "-silent", ...extra], { encoding: "utf-8", timeout: 180000, maxBuffer: 2 * 1024 * 1024 });
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[nuclei] No findings for ${target}`;
        const result = [`🔬 Nuclei: ${target}`, `Findings: ${lines.length}`];
        result.push(...lines.slice(0, 100).map(l => `  ${l}`));
        if (lines.length > 100)
            result.push(`  ... and ${lines.length - 100} more`);
        return result.join("\n");
    }
    catch (e) {
        return `[nuclei Error] ${e.message}`;
    }
}
async function amass(input) {
    if (!input || !input.trim())
        return `[amass] Usage: @amass|<domain> [mode] [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["amass"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
            return `[amass] NOT INSTALLED`;
        }
        const parts = input.trim().split(/\s+/);
        const domain = parts[0];
        ...
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[amass] 0 results for ${domain}`;
        const names = lines.map(l => { try {
            return JSON.parse(l).name || l;
        }
        catch {
            return l;
        } }).filter(Boolean);
...
async function gau(input) {
    if (!input || !input.trim())
        return `[gau] Usage: @gau|<domain> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["gau"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[gau] 0 URLs for ${domain}`;
        const unique = [...new Set(lines)];
        const result = [`🔎 Gau: ${domain}`, `URLs: ${unique.length}`];
        result.push(...unique.slice(0, 80).map(u => `  ${u}`));
        if (unique.length > 80)
            result.push(`  ... and ${unique.length - 80} more`);
        return result.join("\n");
    }
    catch (e) {
        return `[gau Error] ${e.message}`;
    }
}
async function dnsx(input) {
    if (!input || !input.trim())
        return `[dnsx] Usage: @dnsx|<domain_or_file> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["dnsx"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[dnsx] 0 records for ${target}`;
        return [`🔎 Dnsx: ${target}`, `Records: ${lines.length}`, ...lines.slice(0, 50).map(l => `  ${l}`)].join("\n");
    }
    catch (e) {
        return `[dnsx Error] ${e.message}`;
    }
}
async function gitleaks(input) {
    if (!input || !input.trim())
        return `[gitleaks] Usage: @gitleaks|<path> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["gitleaks"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[gitleaks] 0 secrets in ${path}`;
        const result = [`🔐 Gitleaks: ${path}`, `Findings: ${lines.length}`];
        result.push(...lines.slice(0, 50).map(l => `  ${l}`));
        if (lines.length > 50)
            result.push(`  ... and ${lines.length - 50} more`);
        return result.join("\n");
    }
    catch (e) {
        return `[gitleaks Error] ${e.message}`;
    }
}
async function s3scanner(input) {
    if (!input || !input.trim())
        return `[s3scanner] Usage: @s3scanner|<bucket_or_file> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["s3scanner"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
            return `[s3scanner] NOT INSTALLED`;
        }
        const parts = input.trim().split(/\s+/);
        const cmd = parts[0];
        const extra = parts.slice(1);
        ...
        return `[s3scanner Error] ${e.message}`;
    }
}
async function gobuster(input) {
    if (!input || !input.trim())
        return `[gobuster] Usage: @gobuster|<mode>|<url> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["gobuster"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        if (parts.length < 2)
            return `[gobuster] Usage: @gobuster|<mode>|<opts>`;
        ...
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[gobuster] No results`;
        return [`⚡ Gobuster ${mode}: ${rest}`, `Results: ${lines.length}`, ...lines.slice(0, 80).map(l => `  ${l}`)].join("\n");
    }
    catch (e) {
        return `[gobuster Error] ${e.message}`;
    }
}
async function nmap(input) {
    if (!input || !input.trim())
        return `[nmap] Usage: @nmap|<target> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["nmap"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        if (!interesting.length)
            return `[nmap] No results for ${target}`;
        return [`🔎 Nmap: ${target}`, ...interesting.slice(0, 80).map(l => `  ${l}`), ...(interesting.length > 80 ? [`  ... ${interesting.length} total lines`] : [])].join("\n");
    }
    catch (e) {
        return `[nmap Error] ${e.message}`;
    }
}
async function sqlmap(input) {
    if (!input || !input.trim())
        return `[sqlmap] Usage: @sqlmap|<URL> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["sqlmap"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        return `[sqlmap Error] ${e.message}`;
    }
}
async function whatweb(input) {
    if (!input || !input.trim())
        return `[whatweb] Usage: @whatweb|<URL> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["whatweb"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        return `[whatweb Error] ${e.message}`;
    }
}
async function wafw00f(input) {
    if (!input || !input.trim())
        return `[wafw00f] Usage: @wafw00f|<URL> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["wafw00f"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        return `[wafw00f Error] ${e.message}`;
    }
}
async function trufflehog(input) {
    if (!input || !input.trim())
        return `[trufflehog] Usage: @trufflehog|<target> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["trufflehog"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[trufflehog] 0 secrets found`;
        ...
        return `[trufflehog Error] ${e.message}`;
    }
}
async function hydra(input) {
    if (!input || !input.trim())
        return `[hydra] Usage: @hydra|<target>|<proto>|<user>|<pass> [options]`;
    try {
        const { execFileSync, execSync } = await import("child_process");
        try {
            execSync("which hydra", { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        const parts = input.trim().split("|").map(s => s.trim());
        if (parts.length < 3)
            return `[hydra] Need: target|protocol|user|passwords`;
        const target = parts[0];
        const proto = parts[1];
        const user = parts[2];
        const pass = parts[3] || "";
        ...
        if (!pass.includes("/") && !pass.includes("."))
            writeFileSync(passFile, pass.replace(/,/g, "\n"));
        ...
        return `[hydra Error] ${e.message}`;
    }
}
async function masscan(input) {
    if (!input || !input.trim())
        return `[masscan] Usage: @masscan|<target> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["masscan"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
            return `[masscan] NOT INSTALLED (requires root)`;
        }
        const parts = input.trim().split(/\s+/);
        const target = parts[0];
        const extra = parts.slice(1);
        ...
        if (!lines.length)
            return `[masscan] No open ports on ${target}`;
...
async function nikto(input) {
    if (!input || !input.trim())
        return `[nikto] Usage: @nikto|<URL> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["nikto"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
        ...
        if (!findings.length)
            return `[nikto] 0 findings`;
        return [`🔎 Nikto: ${parts[1] || ""}`, `Findings: ${findings.length}`, ...findings.slice(0, 80).map(l => `  ${l}`)].join("\n");
    }
    catch (e) {
        return `[nikto Error] ${e.message}`;
    }
}
async function arjun(input) {
    if (!input || !input.trim())
        return `[arjun] Usage: @arjun|<URL> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["arjun"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
            return `[arjun] NOT INSTALLED (pip install arjun)`;
        }
        const parts = input.trim().split(/\s+/);
        ...
        return `[arjun Error] ${e.message}`;
    }
}
async function gospider(input) {
    if (!input || !input.trim())
        return `[gospider] Usage: @gospider|<URL> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["gospider"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
            return `[gospider] NOT INSTALLED`;
        }
        const parts = input.trim().split(/\s+/);
        const out = execFileSync("gospider", parts, { encoding: "utf-8", timeout: 180000, maxBuffer: 2 * 1024 * 1024 });
        const lines = out.trim().split("\n").filter(Boolean);
        if (!lines.length)
            return `[gospider] 0 results`;
        ...
        if (urls.length > 60)
            result.push(`  ... and ${urls.length - 60} more`);
        return result.join("\n");
    }
    catch (e) {
        return `[gospider Error] ${e.message}`;
    }
}
async function cloud_enum(input) {
    if (!input || !input.trim())
        return `[cloud_enum] Usage: @cloud_enum|<keyword> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["cloud_enum"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
            return `[cloud_enum] NOT INSTALLED (pip install cloud_enum)`;
        }
        const parts = input.trim().split(/\s+/);
        const kw = parts[0];
        const extra = parts.slice(1);
        ...
        if (!found.length)
            return `[cloud_enum] No resources for "${kw}"`;
        return [`☁️ Cloud: ${kw}`, `Found: ${found.length}`, ...found.slice(0, 60).map(l => `  ${l}`)].join("\n");
    }
    catch (e) {
        return `[cloud_enum Error] ${e.message}`;
    }
}
async function notify(input) {
    if (!input || !input.trim())
        return `[notify] Usage: @notify|<message> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["notify"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
            return `[notify] NOT INSTALLED`;
        }
...
async function interactsh(input) {
    if (!input || !input.trim())
        return `[interactsh] Usage: @interactsh|<action> [options]`;
    try {
        const { execFileSync } = await import("child_process");
        try {
            execFileSync("which", ["interactsh-client"], { encoding: "utf-8", timeout: 5000 });
        }
        catch {
            return `[interactsh] NOT INSTALLED`;
        }
        const parts = input.trim().split(/\s+/);
        ...
        if (action === "start") {
            ...
            return `[interactsh] Stopped`;
        return `[interactsh] start | poll <url> | stop`;
    }
    catch (e) {
        return `[interactsh Error] ${e.message}`;
    }
}
// ── END NEW TOOLS ─────────────────────────────────────────
...
async function portScan(target) {
    ...
    for (let i = 0; i < ports.length; i += concurrency) {
        ...
        const scans = batch.map(port => new Promise(resolve => {
    ...
    if (results.length === 1)
        results.push("  (all filtered/closed)");
    return results.join("\n");
}
...
async function sslCheck(host) {
    ...
    return new Promise(resolve => {
        ...
        socket.on("error", (e) => resolve(`[SSL Error] ${e.message}`));
...
async function subdomainEnum(domain) {
    try {
        ...
        if (!r.ok)
            return `[crt.sh Error] HTTP ${r.status}`;
        const data = await r.json();
        if (!Array.isArray(data) || data.length === 0)
            return `(no subdomains found for ${clean})`;
...
async function vtCheck(hash) {
    const key = process.env.VT_API_KEY || "";
    if (!key)
        return "[VT] Set VT_API_KEY env var (free: https://virustotal.com)";
    try {
        ...
        if (r.status === 404)
            return `[VT] Hash "${hash}" not found in VirusTotal database`;
        if (!r.ok)
            return `[VT Error] HTTP ${r.status}`;
        const data = (await r.json());
        ...
        if (attrs.names) {
            lines.push(`\n── Known As ──`);
            attrs.names.slice(0, 10).forEach(n => lines.push(`  ${n}`));
        }
        return lines.join("\n");
    }
    catch (e) {
...
async function cveSearch(query) {
    try {
        const r = await fetch(`https://services.nvd.nist.gov/rest/json/cves/2.0?keywordSearch=${encodeURIComponent(query)}&resultsPerPage=12`, { signal: AbortSignal.timeout(20000), headers: { "User-Agent": "Phantom/1.0" } });
        if (!r.ok)
            return `[NVD Error] HTTP ${r.status}`;
        const data = await r.json();
        ...
        for (const v of vulns.slice(0, 15)) {
            ...
            if (desc)
                lines.push(`  ${desc.substring(0, 180)}`);
...
// ── SEARCHSPLOIT ──────────────────────────────────────────
async function searchsploit(query) {
    ...
    try {
        ...
        const html = await r.text();
        const results = [`🔧 Exploit search results for "${query}":`];
...
// ── BRUTEFORCE ────────────────────────────────────────────
async function bruteforceFn(input) {
...
async function tryLogin(protocol, target, user, pass) {
    switch (protocol) {
        case "ssh": {
            try {
                execSync(`sshpass -p '${pass.replace(/'/g, "'\\''")}' ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 -o BatchMode=no ${user}@${target} 'id' 2>/dev/null`, { encoding: "utf-8", timeout: 10000 });
                return "✅ SUCCESS";
            }
            catch {
                return "❌ failed";
        ...
        case "mysql": {
            try {
                const r = execSync(`mysql -u '${user}' -p'${pass.replace(/'/g, "'\\''")}' -h '${target}' -e 'SELECT 1' --connect-timeout=5 2>/dev/null`, { encoding: "utf-8", timeout: 10000 });
...
async function fileRead(path) {
    try {
        ...
        if (!existsSync(resolved))
            return `[File Error] Not found: ${path}`;
        const content = readFileSync(resolved, "utf-8");
        if (content.length > 100000)
            return `[File Error] Too large (>100KB): ${path} (${content.length} chars)`;
        return content;
    }
    catch (e) {
        if (e.code === "EISDIR")
            return `[File Error] Is a directory: ${path}`;
...
async function fileWrite(input) {
    ...
    if (idx === -1)
        return "[File Write] Usage: path|content\nSeparate path and content with |";
...
async function fileEdit(input) {
    ...
    if (parts.length < 3)
        return "[File Edit] Usage: path|old_string|new_string";
    const [path, oldStr, ...rest] = parts;
    const newStr = rest.join("|");
    try {
        const resolved = resolve(path);
        if (!existsSync(resolved))
            return `[File Edit] Not found: ${path}`;
        let content = readFileSync(resolved, "utf-8");
        if (!content.includes(oldStr))
            return `[File Edit] String not found in ${path}`;
...
async function fileSearch(input) {
    ...
    if (!pattern)
        return "[File Search] Usage: [path|]pattern";
    try {
        const r = execSync(`rg -rn '${pattern.replace(/'/g, "'\\''")}' '${searchPath}' 2>/dev/null | head -40`, { encoding: "utf-8", timeout: 15000, maxBuffer: 1024 * 1024 });
        if (!r.trim())
...
async function fileList(path) {
    try {
        const resolved = resolve(path || ".");
        if (!existsSync(resolved))
            return `[File List] Not found: ${path}`;
...
async function selfInfo() {
    try {
        const pkg = existsSync(resolve(PHANTOM_DIR, "package.json"))
            ...
            for (const e of entries) {
                ...
                try {
                    if (statSync(full).isDirectory())
                        walk(full);
                    else if (e.endsWith(".ts") || e.endsWith(".mjs") || e.endsWith(".json"))
                        srcFiles.push(full.replace(PHANTOM_DIR, "."));
                }
                catch { }
            }
        }
...
async function selfRead(path) {
    try {
        const resolved = resolve(PHANTOM_DIR, path.replace(/^\.\//, ""));
        if (!resolved.startsWith(PHANTOM_DIR))
            return `[Self Read] Access denied: path outside project`;
        if (!existsSync(resolved))
            return `[Self Read] Not found: ${path}`;
        const content = readFileSync(resolved, "utf-8");
...
async function selfEdit(input) {
    const parts = input.split("|");
    if (parts.length < 3)
        return "[Self Edit] Usage: relative_path|old_string|new_string";
    const [relPath, oldStr, ...rest] = parts;
    const newStr = rest.join("|");
    try {
        const resolved = resolve(PHANTOM_DIR, relPath.replace(/^\.\//, ""));
        if (!resolved.startsWith(PHANTOM_DIR))
            return `[Self Edit] Access denied: path outside project`;
        if (!existsSync(resolved))
            return `[Self Edit] Not found: ${relPath}`;
        let content = readFileSync(resolved, "utf-8");
        if (!content.includes(oldStr))
            return `[Self Edit] String not found in ${relPath}`;
        content = content.replace(oldStr, newStr);
        writeFileSync(resolved, content, "utf-8");
...
// ── REPORT SAVE ──────────────────────────────────────────
async function reportSave(input) {
    const parts = input.split("|");
...
async function codeGen(input) {
    ...
    if (!apiKey) {
        ...
        if (outPath) {
            writeFileSync(resolve(outPath), stub, "utf-8");
            return `✅ Stub written to ${outPath}`;
        }
        return stub;
    }
    try {
        ...
        if (outPath) {
            writeFileSync(resolve(outPath), code, "utf-8");
            return `✅ Generated ${code.length} chars → ${outPath}`;
        }
        return code;
    }
    catch (e) {
        return `[Code Gen Error] ${e.message}`;
    }
}
// ── SELF ADD TOOL (Auto-Integrate) ─────────────────────────
async function selfAddTool(prompt) {
    const apiKey = process.env.OPENAI_API_KEY;
    if (!apiKey)
        return "[Self Add Tool] Requires OPENAI_API_KEY";
    try {
        const r = await fetch("https://api.openai.com/v1/chat/completions", {
            method: "POST",
            headers: { "Content-Type": "application/json", Authorization: `Bearer ${apiKey}` },
        ...
        if (ts.includes(`async function ${toolName}(`))
            return `[Self Add] "${toolName}" already exists in TS source`;
        const exportPos = ts.indexOf("export const hackerTools: Record<string, HackerTool> = {");
        if (exportPos === -1)
            return "[Self Add] Cannot find TS insertion point";
        ts = ts.slice(0, exportPos) + "\n" + result.trim() + "\n\n" + ts.slice(exportPos);
        const closePos = ts.lastIndexOf("\n};");
        if (closePos === -1)
            return "[Self Add] Cannot find TS closing";
        ...
        if (mjs.includes(`  ${toolName}: async`))
            return `[Self Add] "${toolName}" already exists in MJS source`;
        ...
        if (mjsClose === -1)
            return "[Self Add] Cannot find MJS insertion point";
...
async function knowledgeAdd(input) {
    try {
        ...
        return `[Knowledge Error] ${e.message}`;
    }
}
async function knowledgeSearch(input) {
    try {
        if (!existsSync(KNOWLEDGE_DIR))
            return "[Knowledge] Empty — add entries with knowledge_add";
        ...
        for (const f of files) {
            ...
            if (!q || data.tags.some((t) => t.toLowerCase().includes(q)) || data.content.toLowerCase().includes(q)) {
                results.push(`[${data.tags.join(", ")}] ${data.content.substring(0, 200)}`);
            }
        }
        if (results.length === 0)
            return `[Knowledge] No results for "${q}"`;
...
async function ensureBuiltinPlaybooks() {
...
async function playbookCreate(input) {
    try {
        ...
        if (apiKey && !stepsRaw.startsWith("shell")) {
            try {
                ...
                const data = await r.json();
                try {
                    const pb = JSON.parse(data?.choices?.[0]?.message?.content || "{}");
                    if (pb.steps) {
                        pb.name = name;
                        pb.description = desc;
                        writeFileSync(resolve(PLAYBOOKS_DIR, `${name}.json`), JSON.stringify(pb, null, 2), "utf-8");
                        return `✅ LLM-created playbook: ${name} (${pb.steps.length} steps)`;
                    }
                }
                catch { }
            }
        ...
        const pb = { name, description: desc, variables: ["target"], steps };
        writeFileSync(resolve(PLAYBOOKS_DIR, `${name}.json`), JSON.stringify(pb, null, 2), "utf-8");
        return `📋 Playbook created: ${name} (${steps.length} steps)`;
    }
    catch (e) {
        return `[Playbook Error] ${e.message}`;
    }
}
async function playbookList() {
    try {
        ...
        if (files.length === 0)
            return "[Playbook] No playbooks found";
...
async function playbookRun(input) {
    try {
        await ensureBuiltinPlaybooks();
        const parts = input.split("|");
        const name = parts[0]?.trim();
        if (!name)
            return "[Playbook] Usage: playbook_run|name|var1=val1,var2=val2";
        const fp = resolve(PLAYBOOKS_DIR, `${name.replace(/[^a-z0-9_-]/gi, "_")}.json`);
        if (!existsSync(fp))
            return `[Playbook] Not found: ${name}`;
        const pb = JSON.parse(readFileSync(fp, "utf-8"));
        // Parse variable assignments
        const vars = {};
        if (parts[1]) {
            parts[1].split(",").forEach(p => { const [k, v] = p.split("="); if (k && v)
                vars[k.trim()] = v.trim(); });
        }
        const log = [`Executing playbook: ${pb.name}`, `Description: ${pb.description}`, `---`, ``];
        for (let i = 0; i < pb.steps.length; i++) {
            const step = pb.steps[i];
            let args = step.args;
            for (const [k, v] of Object.entries(vars))
                args = args.replace(new RegExp(`\\{\\{${k}\\}\\}`, "g"), v);
            log.push(`Step ${i + 1}/${pb.steps.length}: @${step.tool}|${args} — ${step.desc}`);
            try {
                const tool = hackerTools[step.tool];
                if (!tool) {
                    log.push(`  ⚠ Unknown tool: ${step.tool}`);
                    continue;
                }
                const result = await tool.execute(args);
                log.push(`  ${result.substring(0, 1000)}`);
                if (result.length > 1000)
                    log.push(`  … (${result.length} total chars)`);
            }
            catch (e) {
...
async function playbookEdit(input) {
    try {
        const parts = input.split("|");
        const name = parts[0]?.trim();
        if (!name)
            return "[Playbook] Usage: playbook_edit|name|step_index|new_tool|new_args|new_desc";
        const fp = resolve(PLAYBOOKS_DIR, `${name.replace(/[^a-z0-9_-]/gi, "_")}.json`);
        if (!existsSync(fp))
            return `[Playbook] Not found: ${name}`;
        const pb = JSON.parse(readFileSync(fp, "utf-8"));
        if (parts[1] === "desc" && parts[2]) {
            pb.description = parts[2];
            writeFileSync(fp, JSON.stringify(pb, null, 2), "utf-8");
            return `✅ Description updated`;
        }
        if (parts[1] === "add" && parts[2]) {
            pb.steps.push({ tool: parts[2], args: parts[3] || "", desc: parts[4] || parts[2] });
            writeFileSync(fp, JSON.stringify(pb, null, 2), "utf-8");
            return `✅ Step added`;
        }
        const idx = parseInt(parts[1]) - 1;
        if (isNaN(idx) || idx < 0 || idx >= pb.steps.length)
            return `[Playbook] Invalid step index. Steps: 1-${pb.steps.length}`;
        if (parts[2])
            pb.steps[idx].tool = parts[2];
        if (parts[3])
            pb.steps[idx].args = parts[3];
        if (parts[4])
            pb.steps[idx].desc = parts[4];
        writeFileSync(fp, JSON.stringify(pb, null, 2), "utf-8");
...
async function geoip(input) {
    try {
        ...
        if (d.status === "fail")
            return `[GeoIP] ${d.message || "Unknown"}`;
        return [
            `🌍 GeoIP: ${d.query}`,
            `Country: ${d.country}`,
            `Region: ${d.regionName}`,
        ...
        return `[GeoIP Error] ${e.message}`;
    }
}
async function dnsZoneTransfer(input) {
    try {
        ...
        if (nsList.length === 0)
            return "[DNS Zone] No NS records found";
        const results = [`Testing ${nsList.length} name servers for zone transfer on ${domain}:`, ``];
        for (const ns of nsList) {
            try {
                ...
                if (data?.Answer?.length > 0) {
                    results.push(`⚠ VULNERABLE: ${ns} returned ${data.Answer.length} records!`);
                    data.Answer.forEach((a) => results.push(`  ${a.name} ${a.type} ${a.data}`));
                }
                else
                    results.push(`✅ ${ns} — zone transfer denied`);
            }
            catch {
                results.push(`⏰ ${ns} — timeout/error`);
            }
        }
        return results.join("\n");
    }
    catch (e) {
        return `[DNS Zone Error] ${e.message}`;
    }
}
async function httpMethods(input) {
    try {
        ...
        for (const method of methods) {
            try {
                ...
                results.push(`  ${method} → Error: ${e.message.substring(0, 60)}`);
            }
        }
        return results.join("\n");
    }
    catch (e) {
        return `[HTTP Methods Error] ${e.message}`;
    }
}
async function robotsTxt(input) {
    try {
        const baseUrl = input.startsWith("http") ? input : `https://${input}`;
        const url = `${baseUrl.replace(/\/+$/, "")}/robots.txt`;
        const r = await fetch(url, { signal: AbortSignal.timeout(8000) });
        if (r.status === 404)
            return `[robots.txt] Not found at ${url}`;
        const text = await r.text();
        ...
        return `[robots.txt Error] ${e.message}`;
    }
}
async function emailVerify(input) {
    try {
        ...
        if (!emailRegex.test(email))
            return `[Email] Invalid format: ${email}`;
        ...
        if (mxRecords.length === 0)
            return `[Email] ✅ Format valid, but no MX records for ${domain} — domain may not accept mail`;
        const mxList = mxRecords.map((a) => a.data.replace(/\.$/, "")).join(", ");
...
// ── REVERSE DNS ──
async function reverseDns(input) {
    try {
        const ip = input.trim();
        ...
        return `[Reverse DNS Error] ${e.message}`;
    }
}
// ── WAYBACK MACHINE ──
async function wayback(input) {
    try {
        const domain = input.replace(/^https?:\/\//, "").replace(/\/.*$/, "").trim();
        ...
        if (!Array.isArray(d) || d.length < 2)
            return `[Wayback] No snapshots for ${domain}`;
        return `🗄 Wayback: ${domain} (${d.length - 1} snapshots)\n${d.slice(1).map((row) => `  ${row[0].substring(0, 8)} ${row[2] || "—"} ${row[1]}`).join("\n")}`;
    }
    catch (e) {
        return `[Wayback Error] ${e.message}`;
    }
}
// ── CERT EXPIRY ──
async function certExpiry(input) {
    try {
        const domain = input.replace(/^https?:\/\//, "").replace(/\/.*$/, "").trim();
        ...
        if (!raw.trim())
            return `[Cert Expiry] No cert for ${domain}`;
        ...
        return `[Cert Expiry Error] ${e.message}`;
    }
}
// ── CORS TEST ──
async function corsTest(input) {
    try {
        const url = input.startsWith("http") ? input : `https://${input}`;
        ...
        for (const origin of origins) {
            try {
                ...
                r.push(`  Origin: ${origin} → ${(e.message || "").substring(0, 50)}`);
            }
        }
        if (r.some(l => l.includes("*") || (l.includes("evil.com") && l.includes("https://evil.com"))))
...
// ── JWT DECODE ──
async function jwtDecode(input) {
    try {
        const parts = input.trim().split(".");
        if (parts.length !== 3)
            return `[JWT] Expected 3 parts, got ${parts.length}`;
        const b64u = (s) => s.replace(/-/g, "+").replace(/_/g, "/");
        const decode = (s) => {
            try {
                return JSON.stringify(JSON.parse(Buffer.from(b64u(s), "base64").toString()), null, 2);
            }
            catch {
                return Buffer.from(b64u(s), "base64").toString();
            }
        };
        return `🔐 JWT\n── Header ──\n${decode(parts[0])}\n── Payload ──\n${decode(parts[1])}\n── Signature ──\n${parts[2].substring(0, 40)}…`;
    }
    catch (e) {
        return `[JWT Error] ${e.message}`;
    }
}
// ── HASH CRACK ──
async function hashCrack(input) {
    try {
        ...
        if (/^[a-f0-9]{32}$/i.test(hash)) {
            ...
            const text = await r.text();
            if (text?.trim())
                return `🔓 MD5 cracked: ${hash} → ${text.trim()}`;
        }
    ...
    try {
        ...
        const results = (await Promise.allSettled(paths.map(async (p) => {
            try {
                ...
                if (r.status !== 404)
                    return `  ${r.status} ${url}${p}`;
            }
            catch { }
            return null;
        }))).flatMap(r => r.status === "fulfilled" && r.value ? [r.value] : []);
        if (!results.length)
            return `[DirBrute] No interesting paths at ${url}`;
        return `🔍 Dir Bruteforce — ${results.length} hits on ${url}\n${results.join("\n")}`;
    }
    catch (e) {
        return `[DirBrute Error] ${e.message}`;
    }
}
/** XSS Scanner — injects payloads, checks for reflection */
async function xssScan(input) {
    try {
        const url = input.startsWith("http") ? input : `https://${input}`;
        ...
        const results = (await Promise.allSettled(payloads.map(async (p) => {
            try {
                ...
                const text = await r.text();
                if (text.includes(p.substring(0, 15)))
                    return `  ⚠ ${p.substring(0, 25)} reflected at ${testUrl.substring(0, 60)}`;
            }
            catch { }
            return null;
        }))).flatMap(r => r.status === "fulfilled" && r.value ? [r.value] : []);
        if (!results.length)
            return `[XSS] No obvious reflection at ${url}`;
        return `🚨 XSS — ${results.length} reflection(s) on ${url}\n${results.join("\n")}`;
    }
    catch (e) {
        return `[XSS Error] ${e.message}`;
    }
}
/** SQLi Detection — sends SQLi payloads, checks error signatures */
async function sqlDetect(input) {
    try {
        const url = input.startsWith("http") ? input : `https://${input}`;
        ...
        const results = (await Promise.allSettled(payloads.map(async (p) => {
            try {
                ...
                if (sigs.some(s => text.toLowerCase().includes(s)))
                    return `  ⚠ ${p.substring(0, 20)} → SQL error at ${testUrl.substring(0, 50)}`;
            }
            catch { }
            return null;
        }))).flatMap(r => r.status === "fulfilled" && r.value ? [r.value] : []);
        if (!results.length)
            return `[SQLi] No SQL errors at ${url}`;
        return `⚠️ SQLi — ${results.length} potential injection(s) on ${url}\n${results.join("\n")}`;
    }
    catch (e) {
        return `[SQLi Error] ${e.message}`;
    }
}
/** Open Redirect — tests common redirect params for external redirects */
async function openRedirect(input) {
    try {
        const url = input.startsWith("http") ? input : `https://${input}`;
        ...
        const results = (await Promise.allSettled(params.map(async (p) => {
            try {
                ...
                if ((r.headers.get("location") || "").includes("evil.com"))
                    return `  ⚠ ${p}= → external redirect`;
            }
            catch { }
            return null;
        }))).flatMap(r => r.status === "fulfilled" && r.value ? [r.value] : []);
        if (!results.length)
            return `[OpenRedirect] No vulnerable params at ${url}`;
        return `🔀 Redirect Check — ${results.length} open redirect(s)\n${results.join("\n")}`;
    }
    catch (e) {
        return `[OpenRedirect Error] ${e.message}`;
    }
}
// ── OSINT TOOLS ───────────────────────────────────────────
/** Shodan Search — requires SHODAN_API_KEY */
async function shodanSearch(input) {
    const key = process.env.SHODAN_API_KEY;
    if (!key)
        return `[Shodan] Set SHODAN_API_KEY env var. Get one at https://account.shodan.io`;
    try {
        ...
        const d = await r.json();
        if (!d.matches?.length)
            return `[Shodan] No results for "${input}"`;
        const lines = d.matches.slice(0, 10).map((m) => `  ${m.ip_str}:${m.port} ${m.transport || ""} ${(m.product || m.data || "").substring(0, 50)}`);
        return `🌐 Shodan — ${d.total} result(s) for "${input}"\n${lines.join("\n")}`;
    }
    catch (e) {
        return `[Shodan Error] ${e.message}`;
    }
}
/** Email Breach Check — tries HIBP API (needs HIBP_API_KEY) */
async function emailBreach(input) {
    try {
        const email = input.trim().toLowerCase();
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email))
            return `[EmailBreach] Invalid email`;
        const key = process.env.HIBP_API_KEY;
        if (!key)
            return `[EmailBreach] Set HIBP_API_KEY for breach lookups. Get one at haveibeenpwned.com/API/Key`;
        ...
        if (r.status === 404)
            return `🔒 ${email} — No known breaches ✅`;
        if (r.status === 200) {
            const breaches = await r.json();
            return `⚠️ ${email} — ${breaches.length} breach(es)\n${breaches.slice(0, 10).map((b) => `  🔴 ${b.Name} (${b.BreachDate || "?"})`).join("\n")}`;
        }
        return `[EmailBreach] HTTP ${r.status}`;
    }
    catch (e) {
        return `[EmailBreach Error] ${e.message}`;
    }
}
/** GitHub Dork — searches code for secrets/keys */
async function githubDork(input) {
    try {
        ...
        if (r.status === 403)
            return `[GitHubDork] Rate limited. Authenticated: 30 req/min, unauthenticated: 10. Try again.`;
        const d = await r.json();
        if (!d.items?.length)
            return `[GitHubDork] No results for "${input}"`;
        return `🔍 GitHub Dork — ${d.total_count} result(s) for "${input}"\n${d.items.slice(0, 10).map((i) => `  📄 ${i.repository?.full_name || "?"}/${i.name}`).join("\n")}`;
    }
    catch (e) {
        return `[GitHubDork Error] ${e.message}`;
    }
}
/** Subdomain Takeover Check — tests CNAME for unclaimed cloud services */
async function subTakeover(input) {
    try {
        const domain = input.replace(/^https?:\/\//, "").replace(/\/.*$/, "").trim();
        ...
        if (!cnames.length)
            return `[SubTakeover] No CNAME records for ${domain}`;
        const svcs = {
        ...
        const lines = cnames.map((c) => {
            const svc = Object.entries(svcs).find(([s]) => c.includes(s));
            return svc ? `  ⚠ → ${c.trim()} — ${svc[1]} takeover possible!` : `  ℹ → ${c.trim()}`;
        });
        return `🔍 Subdomain Takeover — ${domain}\n${lines.join("\n")}`;
    }
    catch (e) {
        return `[SubTakeover Error] ${e.message}`;
    }
}
// ── PLUGIN SYSTEM ─────────────────────────────────────────
...
async function pluginLoad(input) {
    try {
        ...
        if (!files.length)
            return `[Plugin] No plugins in ${pluginDir}. Use @plugin_create to make one.`;
        let loaded = 0;
        for (const f of files) {
            try {
                const mod = await import(pathToFileURL(resolve(pluginDir, f)).href);
...
async function reportExport(input) {
    try {
        ...
        return `[ReportExport Error] ${e.message}`;
    }
}
export const hackerTools = {
    shell: {
        description: "Execute ANY shell command on the system. Use for: running tools, scripts, file operations, network scans, system info, package management. Input: shell command string.",
        execute: shell,
    },
    web_fetch: {
        description: "Fetch a URL and return its content (HTML stripped, plain text). Use for: reading web pages, APIs, documentation, checking endpoints. Input: full URL including https://.",
        execute: webFetch,
    },
    decode: {
        description: "Auto-detect and decode encoded strings. Tries base64, hex, URL encoding, binary, and ROT13. Use for: decoding obfuscated strings, payloads, and encoded data. Input: the encoded string.",
        execute: decode,
    },
    file_analyze: {
        description: "Deep file analysis: file type detection by magic bytes, MD5/SHA1/SHA256 hashes, entropy calculation (detects packed/encrypted malware), and printable string extraction. Use for: malware analysis, file forensics, verifying file integrity. Input: absolute file path.",
        execute: fileAnalyze,
    },
    dns_lookup: {
        description: "DNS reconnaissance: resolves A, AAAA, MX, NS, TXT, CNAME, and SOA records for a domain. Use for: OSINT, domain recon, infrastructure discovery. Input: domain name (no http://).",
        execute: dnsLookup,
    },
    hash: {
        description: "Compute MD5, SHA1, SHA256 hash of text or a file. Use for: integrity checks, verifying downloads, fingerprinting. Input: text string or file path.",
        execute: hash,
    },
    // ── NEW TOOLS ──
    whois: {
        description: "WHOIS lookup for a domain. Returns registrar, dates, name servers, and contact info. Use for: OSINT, domain investigation, ownership discovery. Input: domain name (no http://).",
        execute: whois,
    },
    port_scan: {
        description: "TCP port scan a host. Scans common ports (FTP, SSH, HTTP, etc.) by default. Custom: host:port1,port2 or host:start-end. Use for: network recon, vulnerability assessment. Input: hostname or IP, optionally with port range.",
        execute: portScan,
    },
    http_headers: {
        description: "Fetch HTTP response headers from a URL using HEAD request. Shows status, security headers, server info, content-type. Use for: web recon, security header audit. Input: full URL including https://.",
        execute: httpHeaders,
    },
    ssl_check: {
        description: "Check SSL/TLS certificate details for a host. Shows issuer, validity, cipher, SANs, days remaining. Use for: certificate expiry monitoring, TLS security audit. Input: hostname (optionally :port, default 443).",
        execute: sslCheck,
    },
    sub_enum: {
        description: "Enumerate subdomains via certificate transparency logs (crt.sh). Use for: OSINT, attack surface mapping, discovering hidden assets. Input: domain name (no http://).",
        execute: subdomainEnum,
    },
    crawl: {
        description: "Crawl a web page: fetch HTML, extract all links, forms, and script sources. Use for: web recon, asset discovery, spidering. Input: full URL including https://.",
        execute: webCrawl,
    },
    vt_check: {
        description: "Check a file hash against VirusTotal. Shows detection ratio, file type, names, and top malicious engine results. Requires VT_API_KEY env var. Input: MD5/SHA1/SHA256 hash.",
        execute: vtCheck,
    },
    yara: {
        description: "Scan a file with YARA rules. Format: rules_file|target_path, or just target_path. Use for: malware pattern matching, IOC scanning. Requires yara CLI installed. Input: rules_and_target.",
        execute: yaraScan,
    },
    // ── AUTO WORKFLOWS ──
    recon: {
        description: "FULL AUTOMATED RECON: runs WHOIS → DNS → subdomains → HTTP headers → SSL → port scan → crawl. Saves timestamped report to disk. Use for: one-shot surface mapping, asset discovery, target profiling. Input: domain or URL.",
        execute: recon,
    },
    cve_search: {
        description: "Search NVD (National Vulnerability Database) for CVEs matching a query. Shows CVE ID, CVSS score, severity, description, and NVD link. Use for: finding known vulns for software/version. Input: search query (e.g. 'apache 2.4.49' or 'nginx 1.20').",
        execute: cveSearch,
    },
    searchsploit: {
        description: "Search for public exploits. Tries local searchsploit CLI first, falls back to packetstorm. Use for: finding PoC exploits for a vulnerability or software. Input: search query (e.g. 'WordPress 5.8' or 'CVE-2024-XXXX').",
        execute: searchsploit,
    },
    bruteforce: {
        description: "Multi-protocol brute force login. Supports SSH, FTP, HTTP POST, and MySQL. Format: protocol|target|user|pass1,pass2,pass3. Use for: password testing, credential auditing. Input: protocol|target|username|comma-separated-passwords.",
        execute: bruteforceFn,
    },
    // ── FILE TOOLS ──
    file_read: {
        description: "Read the contents of any file on the system. Use for: viewing source code, configs, logs, data files. Shows full content (max 100KB). Input: absolute or relative file path.",
        execute: fileRead,
    },
    file_write: {
        description: "Write content to a file (creates or overwrites). Creates parent directories automatically. Use for: saving generated code, writing scripts, creating config files. Format: path|content — separate path and content with pipe.",
        execute: fileWrite,
    },
    file_edit: {
        description: "Find and replace text in an existing file. Uses exact string matching (not regex). Use for: patching code, modifying configs, fixing errors. Format: path|old_string|new_string — pipe-separated.",
        execute: fileEdit,
    },
    file_search: {
        description: "Search file contents for a text pattern across a directory. Uses ripgrep if available. Use for: finding code references, error messages, configuration values. Format: [directory|]pattern — defaults to current dir.",
        execute: fileSearch,
    },
    file_list: {
        description: "List files and directories in a path. Shows file sizes, dir count. Use for: exploring project structure, finding files, directory recon. Input: directory path (defaults to current dir).",
        execute: fileList,
    },
    // ── SELF TOOLS ──
    self_info: {
        description: "Show Phantom's own information: version, runtime, platform, all available tools, source file tree, LLM connection status, config paths. Use for: self-awareness, capability discovery, debugging.",
        execute: selfInfo,
    },
    self_read: {
        description: "Read Phantom's own source files (locked to project directory). Use for: code review, understanding architecture, self-modification planning. Input: relative path from project root (e.g. 'phantom.mjs' or 'src/core/hacker-tools.ts').",
        execute: selfRead,
    },
    self_edit: {
        description: "Edit Phantom's own source code (locked to project directory). Use for: self-improvement, adding tools, fixing bugs, evolving capabilities. Format: relative_path|old_string|new_string — pipe-separated.",
        execute: selfEdit,
    },
    // ── AUTO SCAN ──
    vuln_scan: {
        description: "FULL AUTOMATED VULNERABILITY SCAN: runs Phase 1 recon (WHOIS/DNS/subdomains/headers/SSL/ports) → Phase 2 CVE search → Phase 3 exploit search → Phase 4 brute force. Saves comprehensive markdown report with all findings. Input: domain or URL.",
        execute: vulnScan,
    },
    report_save: {
        description: "Save text content as a timestamped markdown report file. Use for: documenting findings, saving scan results, creating evidence. Format: name|content — name is the report filename (saved to ~/.config/phantom/reports/).",
        execute: reportSave,
    },
    session_save: {
        description: "Save current Phantom session state to a named session file. Use for: bookmarking work, preserving findings between runs. Input: session name (alphanumeric, underscores/hyphens).",
        execute: sessionSave,
    },
    session_load: {
        description: "Load a previously saved Phantom session. Use for: resuming work, reviewing past findings. Input: session name.",
        execute: sessionLoad,
    },
    code_gen: {
        description: "Generate code using OpenAI LLM (requires OPENAI_API_KEY). Falls back to a stub template. Format: prompt|language|output_path. Use for: writing functions, scripts, exploits, tools autonomously.",
        execute: codeGen,
    },
    self_add_tool: {
        description: "Generate a new Phantom tool using LLM and save it for integration. Requires OPENAI_API_KEY. Input: natural language description of the tool (e.g. 'check if a site uses HSTS header'). Saves generated code to ~/.config/phantom/generated/.",
        execute: selfAddTool,
    },
    // ── KNOWLEDGE BASE ──
    knowledge_add: {
        description: "Add an entry to Phantom's persistent knowledge base. Use for: saving findings, techniques, commands for future reference. Format: tag1,tag2|content. Searchable with knowledge_search.",
        execute: knowledgeAdd,
    },
    knowledge_search: {
        description: "Search Phantom's knowledge base by keyword or tag. Use for: retrieving past findings, techniques, learned information. Input: search query.",
        execute: knowledgeSearch,
    },
    // ── PLAYBOOK SYSTEM ──
    playbook_create: {
        description: "Create a new executable playbook (multi-step automation script). Uses LLM if OPENAI_API_KEY set. Format: name|description|step1,step2,... or just name|description for LLM generation.",
        execute: playbookCreate,
    },
    playbook_list: {
        description: "List all available playbooks with descriptions and step counts. Includes 4 built-in playbooks plus custom. Playbooks can chain other playbooks via playbook_run steps.",
        execute: playbookList,
    },
    playbook_run: {
        description: "Execute a playbook against a target. Steps run sequentially with variable substitution ({{target}}). Supports chaining: use playbook_run step to call another playbook. Input: playbook_name|target=example.com,port=80. Returns full execution log.",
        execute: playbookRun,
    },
    playbook_edit: {
        description: "Modify a playbook: edit steps, change description, or append new steps. Format: name|step_num|tool|args|desc or name|desc|new_description or name|add|tool|args|desc.",
        execute: playbookEdit,
    },
    // ── NEW TOOLS ──
    sandbox: {
        description: "SAFELY RUN a suspicious binary in an isolated sandbox environment. Creates temp directory, strace-monitor if available, captures syscalls (network/file/process), stdout/stderr, exit code, and SHA256 hash. Detects PE vs ELF vs script. Use for: malware dynamic analysis, behavioral testing, I/O monitoring. Format: path|args|timeout(seconds). Example: @sandbox|/tmp/suspicious.bin||30",
        execute: sandboxExec,
    },
    pe_analyze: {
        description: "Deep PE (Portable Executable) format analysis: DOS header, COFF header (machine/compile time/sections), optional header (entry point/image base/subsystem), section table with W+X detection, data directories, entropy calculation, packer detection (UPX/Themida/VMP). Use for: static malware analysis of Windows binaries, unpacking detection. Input: absolute path to PE file.",
        execute: peAnalyze,
    },
    elf_analyze: {
        description: "Deep ELF format analysis: class (32/64-bit), byte order, OS/ABI, type (EXEC/DYN/REL), machine arch, entry point, program headers with W+X detection, dynamic symbols (imported functions), entropy calculation, GNU_STACK checks. Use for: static analysis of Linux binaries, packer/reverse engineering. Input: absolute path to ELF file.",
        execute: elfAnalyze,
    },
    macro_scan: {
        description: "Analyze Office documents for malicious macros/VBA code. Scans OLE2 (.doc/.xls/.ppt), OOXML (.docx/.xlsm/.docm), and raw VBA/VBS scripts. Detects Auto-executing macros (Auto_Open, Document_Open), suspicious patterns (Shell, CreateObject, WScript, PowerShell, cmd.exe). Use for: phishing analysis, malware document triage. Input: path to document or script.",
        execute: macroScan,
    },
    strings_deep: {
        description: "Advanced string extraction with AI-style categorization. Extracts all printable strings (≥4 chars) and classifies into: URLs, email addresses, IP addresses, domains, file paths, potential secrets/keys, and other strings. Includes entropy calculation for packed file detection. Use for: malware string analysis, IOC discovery, binary triage. Input: absolute file path.",
        execute: stringsDeep,
    },
    hex_dump: {
        description: "Classic hex dump viewer with ASCII sidebar. Shows offset (hex), 16 bytes per row in hex format, and ASCII representation. Configurable offset and length. Use for: manual binary analysis, file header inspection, pattern matching. Format: path|offset|length. Default: offset=0, length=512 bytes (max 4096).",
        execute: hexDump,
    },
    extract_ioc: {
        description: "Extract Indicators of Compromise (IOCs) from text or files. Detects: IPv4 addresses, domains, URLs, email addresses, MD5/SHA1/SHA256 hashes, file paths. Filters private/local IPs and deduplicates. Use for: threat intelligence, log analysis, malware report parsing. Input: file path (auto-detected) or raw text.",
        execute: extractIoc,
    },
    proc_analyze: {
        description: "Deep process forensics inspection. Shows: process name/state/PPID/UID/threads, command line, working directory, executable, environment variables (with secrets detection), open file descriptors (sockets/pipes/files), network connections (TCP/UDP with state), memory regions (with RWX detection). Use for: live malware analysis, process investigation, incident response. Input: PID number.",
        execute: procAnalyze,
    },
    // ── RECON TOOLS ──
    geoip: {
        description: "IP geolocation lookup via ip-api.com (free, no key). Shows country, region, city, coordinates, ISP, ASN. Use for: locating target servers, tracing attack origins. Input: IP address or domain.",
        execute: geoip,
    },
    dns_zone: {
        description: "Test DNS zone transfer (AXFR) on all authoritative name servers. Use for: finding DNS misconfigurations that leak internal network structure. Input: domain name.",
        execute: dnsZoneTransfer,
    },
    http_methods: {
        description: "Fuzz HTTP methods on a target URL. Tests GET, POST, PUT, DELETE, OPTIONS, PATCH, HEAD, TRACE, CONNECT. Shows status codes and Allow headers. Input: URL.",
        execute: httpMethods,
    },
    robots_txt: {
        description: "Fetch and analyze robots.txt from a target. Shows all rules, disallowed paths, sitemaps, and comments. Use for: discovering hidden/disallowed endpoints. Input: domain or URL.",
        execute: robotsTxt,
    },
    email_verify: {
        description: "Validate email format and check domain MX records. Use for: verifying email deliverability, recon on email infrastructure. Input: email address.",
        execute: emailVerify,
    },
    // ── UTILITY TOOLS ──
    reverse_dns: {
        description: "Reverse DNS lookup (PTR record) for an IP address. Shows the domain names associated with the IP. Use for: identifying hosts, email server validation. Input: IP address.",
        execute: reverseDns,
    },
    wayback: {
        description: "Query Wayback Machine for historical URL snapshots of a domain. Shows timestamps, status codes, and URLs. Use for: recon on old endpoints, content discovery. Input: domain or URL.",
        execute: wayback,
    },
    cert_expiry: {
        description: "Check SSL certificate expiry date and days remaining. Uses openssl to fetch and parse cert dates. Use for: monitoring cert expiration, security audits. Input: domain name.",
        execute: certExpiry,
    },
    cors_test: {
        description: "Test for CORS misconfigurations by sending requests with various Origin headers. Detects wildcard origins, null origin acceptance, and credential leakage. Input: URL.",
        execute: corsTest,
    },
    jwt_decode: {
        description: "Decode a JWT token header and payload without verification. Shows the JSON content of each part. Use for: analyzing auth tokens, testing JWT implementations. Input: full JWT string.",
        execute: jwtDecode,
    },
    hash_crack: {
        description: "Look up MD5 hash in online rainbow tables (nitrxgen API). Use for: cracking password hashes, identifying plaintext. Input: MD5 hash (32 hex chars).",
        execute: hashCrack,
    },
    // ── WEB APP SECURITY ──
    dir_bruteforce: {
        description: "Web directory brute force: probes 30+ common paths (admin, api, .git, .env, uploads, etc.). Uses parallel requests. Use for: finding hidden endpoints, admin panels, exposed files. Input: URL or domain.",
        execute: dirBruteforce,
    },
    xss_scan: {
        description: "Cross-Site Scripting scanner: injects multiple XSS payloads via query params and checks if they reflect. Use for: detecting XSS vulnerabilities. Input: URL (with or without params).",
        execute: xssScan,
    },
    sql_detect: {
        description: "SQL Injection detection: sends common SQLi payloads (' OR 1=1--, UNION SELECT, etc.) and scans for SQL error signatures. Use for: finding SQLi entry points. Input: URL with parameter.",
        execute: sqlDetect,
    },
    open_redirect: {
        description: "Open redirect scanner: tests 15 common redirect parameters (url, redirect, next, etc.) for external redirects. Use for: finding unvalidated redirects. Input: URL.",
        execute: openRedirect,
    },
    // ── OSINT ──
    shodan_search: {
        description: "Search Shodan for internet-connected devices, open ports, and banners. Requires SHODAN_API_KEY. Use for: OSINT, exposed devices, infrastructure discovery. Input: search query.",
        execute: shodanSearch,
    },
    email_breach: {
        description: "Check if an email appears in known data breaches via HIBP API (requires HIBP_API_KEY). Shows breach names and dates. Use for: OSINT, compromise investigation. Input: email address.",
        execute: emailBreach,
    },
    github_dork: {
        description: "Search GitHub source code for secrets, keys, and sensitive data via GitHub Code Search. Use for: finding exposed credentials, API keys, and configs. Input: search query.",
        execute: githubDork,
    },
    sub_takeover: {
        description: "Check subdomain for potential takeover via unclaimed CNAME records. Checks 15+ cloud services (AWS S3, CloudFront, GitHub Pages, Heroku, Azure, etc.). Use for: identifying dangling DNS. Input: domain.",
        execute: subTakeover,
    },
    // ── PLUGIN SYSTEM ──
    plugin_load: {
        description: "Load external plugin tools from ~/.config/phantom/plugins/. Imports .mjs/.js modules with name/description/execute exports. Use for: extending Phantom without modifying core. Input: optional plugin directory path.",
        execute: pluginLoad,
    },
    plugin_create: {
        description: "Create a new Phantom plugin skeleton. Format: name|description. Saves to ~/.config/phantom/plugins/ and attempts auto-load. Use for: quickly extending toolset. Input: tool_name|description.",
        execute: pluginCreate,
    },
    // ── REPORTING ──
    report_export: {
        description: "Export a saved markdown report to styled dark-themed HTML. Open in browser → Ctrl+P → Save as PDF. Use for: sharing findings, report formatting. Input: report name (without .md).",
...
//# sourceMappingURL=hacker-tools.js.map