...
function createProvider() {
  const PROVIDERS = {
    openai:    { url: "https://opencode.ai/zen/v1",              keyEnv: "OPENCODE_ZEN_API_KEY",   defaultModel: "deepseek-v4-flash-free", chatPath: "/chat/completions", fmt: o => ({ model: o.model, messages: o.messages, temperature: 0.7, max_tokens: 16384 }), parse: d => d.choices?.[0]?.message?.content?.trim() || "...", auth: k => ({ Authorization: `Bearer ${k}` }) },
    ...
    opencode:  { url: "https://opencode.ai/zen/v1",              keyEnv: "OPENCODE_ZEN_API_KEY",   defaultModel: "deepseek-v4-flash-free", chatPath: "/chat/completions", fmt: o => ({ model: o.model, messages: o.messages, temperature: 0.7, max_tokens: 16384 }), parse: d => d.choices?.[0]?.message?.content?.trim() || "...", auth: k => ({ Authorization: `Bearer ${k}` }) },
  };
...
export async function runChat(llm) {
  ...
  console.log(`${c("green")}${_B}PHANTOM Chat${R} ${c("dim")}v${version} · ${llm.provider}${R}`);
    ...
    rl.question(`${c("green")}>> ${R}`, async (input) => {
      ...
      if (trimmed.startsWith("/")) {
        ...
        if (cmd.startsWith("model ")) {
          ...
          console.log(`${c("dim")}Model: ${llm.provider}${R}\n`);
          ask();
          return;
        }
