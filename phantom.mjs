...
class Agent {
  ...
  registerHackerTools() {
    const toolList = {
      ...
      rtsp_check: "Check RTSP port (554) on a target and attempt to get stream info via OPTIONS request. Use for: detecting IP cameras, DVRs, NVRs. Input: host:port (default 554).",
      onvif_discover: "Discover ONVIF-compliant cameras/DVRs on the network via WS-Discovery multicast probe. Use for: finding all ONVIF devices on LAN. Input: optional network (default auto).",
      camera_stream_test: "Test common RTSP stream paths on a camera/DVR. Tries standard paths for Dahua, Hikvision, Axis, etc. Use for: finding working stream URLs. Input: host:port (default 554).",
      nmap_cctv: "Run nmap with CCTV-specific scripts (rtsp-methods, onvif-info, http-title). Scans common CCTV ports. Use for: comprehensive CCTV device enumeration. Input: target IP or CIDR.",
