...
export async function handleApiRequest(req, res) {
  ...
  try {
    ...
    if (url.pathname.startsWith("/api/missions/") && url.pathname.length > 12) {
      const missionId = decodeURIComponent(url.pathname.slice(12));
      const f = resolve(MISSIONS_DIR, `${missionId}.json`);
      if (!fs.existsSync(f)) return json(res, { ok: false, error: "Mission not found" }, 404);
      const mission = JSON.parse(fs.readFileSync(f, "utf-8"));
      return json(res, { ok: true, mission });
    }
    ...
    if (url.pathname === "/api/schedules") {
      ...
      if (fs.existsSync(SCHEDULES_FILE)) {
        try { schedules = JSON.parse(fs.readFileSync(SCHEDULES_FILE, "utf-8")); } catch {}
      }
      return json(res, {
        ok: true,
    ...
    if (url.pathname === "/api/system") {
      ...
      try {
        netInfo = execSync("cat /proc/net/dev", { encoding: "utf-8", timeout: 5000 }).trim();
      } catch {}
      
      // Get disk I/O stats
      let diskIO = "";
      try {
        diskIO = execSync("cat /proc/diskstats", { encoding: "utf-8", timeout: 5000 }).trim();
      } catch {}
