...
function gitPush() {
  if (!process.env.PHANTOM_GIT_PUSH) return false; // opt-in
  const root = gitRoot();
  if (!root) return false;
  try {
    execSync("git push", { cwd: root, encoding: "utf-8", timeout: 60000 });
    return true;
  } catch { return false; }
}
...
export async function autoEvolve() {
  ...
  if (hasCommits && finalTestsOk) {
    try {
      ...
      const root = gitRoot();
      if (root) {
        const behind = execSync(`git log origin..HEAD --oneline 2>/dev/null`, { cwd: root, encoding: "utf-8", timeout: 5000 }).trim();
        if (behind) {
          const count = behind.split("\n").length;
          results.push({ phase: "git", status: "ready_to_push", count, commits: behind,
            summary: `✅ All checks passed (${passed} tests). Ready to push ${count} commit(s).` });
        }
      }
    } catch { /* no remote or nothing pending */ }
  }
