...
export function renderLogo(opts = {}) {
  ...
  // Shadow figlet logo — no box-drawing borders, clean
  const lines = [
    ``,
    `  ${M}${b} _ \\\\  |   |    \\\\     \\\\  |__ __| _ \\\\   \\\\  |${_r}`,
    `  ${M}${b} |   | |   |   _ \\\\     \\\\ |   |  |   | |\\\\/ |${_r}`,
    ...
    ``,
    `  ${C}${b}P${_r} ${M}${b}H${_r} ${Y}${b}A${_r} ${C}${b}N${_r} ${M}${b}T${_r} ${Y}${b}O${_r} ${C}${b}M${_r}`,
  ];
...
export function renderBanner(text = "PHANTOM", subtitle = "") {
  ...
  const _r = _R;
  const b = _B;
  return `${C}${b}${text}${_r}${subtitle ? ` ${dim}${subtitle}${_r}` : ""}`;
}
...
export function createSpinner(startTime) {
  const frames = ['⠋', '⠙', '⠹', '⠸', '⠼', '⠴', '⠦', '⠧', '⠇', '⠏'];
  let i = 0, timer = null;
  const fmt = (ts) => {
  ...
  return {
    start(msg) {
      ...
      timer = setInterval(() => {
        ...
        process.stdout.write(`\r${fg("cyan")}${_B}${f}${_R} [${fmt(startTime)}] ${msg}`);
      }, 80);
    },
    stop() {
      if (!timer) return;
