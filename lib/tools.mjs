...
`;
  ...
  install_missing: async (input) => {
    try {
      ...
      for (const tool of targets.slice(0, 10)) {
      ...
      return `[install] ${results.length} results\n${results.join("\n")}`;
    } catch (e) { return `[install Error] ${e.message}`; }
  },

  // ── SELF INTEGRATE — inject generated module into hackerTools + registry ──
  ...
  graph: async (input) => {
    try {
      ...
      return results.map(r => `  ${r.tool}: ${r.books?.join(", ") || "—"} | ${r.cves?.join(", ") || "—"} | [${(r.tags||[]).join(", ")}]`).join("\n");
