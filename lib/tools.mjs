...
export const hackerTools = {
  ...
  update: async (input) => {
    ...
    try {
      ...
      // Re-check syntax
      try { execSync(`node --check "${resolve(homedir(), "Phantom", "phantom.mjs")}"`, { stdio:"pipe", timeout:5000 }); } catch { return `[update] Pulled but SYNTAX ERROR in new code. Roll back with: git reset --hard HEAD@{1}`; }
...
╚${"═".repeat(36)}╝`;
  ...
  git_sync: async (input) => {
    try {
      ...
      const PHANTOM_DIR = resolve(new URL(".", import.meta.url).pathname, "..");
