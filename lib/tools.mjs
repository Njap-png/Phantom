  ...
  config: async (input) => {
    try {
      ...
      if (cmd === "set") {
              ...
              if (upper.endsWith("_API_KEY")||["VT_API_KEY","SHODAN_API_KEY","GITHUB_TOKEN","HACKERONE_API_TOKEN","CHAOS_API_KEY"].includes(upper)) process.env[upper] = v;
    ...
    try {
      ...
      if (cmd === "push") {
        ...
        try {
          ...
          if (cfg.GIT_USER && cfg.GIT_TOKEN) {
            const remoteUrl = run("git remote get-url origin 2>/dev/null || echo ''").trim();
            if (remoteUrl && remoteUrl.startsWith("https://")) {
              const newUrl = remoteUrl.replace("https://", `https://${cfg.GIT_USER}:${cfg.GIT_TOKEN}@`);
              run(`git remote set-url origin "${newUrl}" 2>/dev/null`);
            }
          }
...
Examples:
  ...
  @hackerone test`;
      ...
      const username = process.env.HACKERONE_API_USERNAME || cfg.HACKERONE_API_USERNAME || cfg.hackerone_api_username;
      const token = process.env.HACKERONE_API_TOKEN || cfg.HACKERONE_API_TOKEN || cfg.hackerone_api_token;
...
Or set env vars HACKERONE_API_USERNAME and HACKERONE_API_TOKEN`;
      ...
      if (cmd === "programs") {
        ...
        return progs.map(p => {
          ...
          return `  ${a.handle} — ${a.name} (${a.state}) | ${a.offers_bounties ? "💰" : "🏅"} | ${a.url}`;
        }).join("\\n");
      }
...
Examples:
  ...
  // ── CCTV/RTSP Tools ─────────────────────────────────────────
  rtsp_check: async (target) => {
    // Check RTSP port and attempt to get stream info
    const [host, portStr] = target.includes(":") ? target.split(":") : [target, "554"];
    const port = parseInt(portStr) || 554;
    ...
    return new Promise(resolve => {
      ...
      socket.on("connect", () => {
        // Send OPTIONS request to test RTSP
      ...
      socket.on("data", (data) => {
        ...
        if (response.includes("RTSP/1.0") || response.includes("Public:")) {
          socket.destroy();
          const lines = [
            `RTSP Server: ${host}:${port}`,
            `Status: Connected`,
      ...
      socket.on("error", (e) => resolve(`[RTSP Error] ${e.message}`));
      socket.on("timeout", () => { socket.destroy(); resolve("[RTSP Error] Timeout"); });
      socket.on("close", () => {
        if (!response) resolve(`[RTSP] Port ${port} open but no RTSP response`);
      });
  ...
  onvif_discover: async (target) => {
    // Discover ONVIF devices via WS-Discovery (UDP multicast)
    ...
    return new Promise(resolve => {
      ...
      const timeout = setTimeout(() => {
        ...
        if (responses.length === 0) {
          resolve("[ONVIF] No devices responded to discovery probe");
        } else {
          resolve(`ONVIF Devices Found:\n${responses.join("\n---\n")}`);
        }
      }, 5000);
      
      socket.on("message", (msg, rinfo) => {
        const text = msg.toString();
        if (text.includes("NetworkVideoTransmitter") || text.includes("onvif")) {
          const ip = rinfo.address;
          const match = text.match(/<d:XAddrs>([^<]+)<\/d:XAddrs>/);
          const xaddrs = match ? match[1] : "unknown";
          responses.push(`IP: ${ip}\nXAddrs: ${xaddrs}\nRaw: ${text.substring(0, 800)}`);
        }
      });
  ...
  camera_stream_test: async (target) => {
    // Test common camera stream paths
    const [host, portStr] = target.includes(":") ? target.split(":") : [target, "554"];
    const port = parseInt(portStr) || 554;
    ...
    const results = [`Camera Stream Test: ${host}:${port}`];
    ...
    for (const path of commonPaths) {
      try {
        ...
        const r = await fetch(`rtsp://${host}:${port}${path}`, {
        ...
        if (r && (r.status === 200 || r.status === 401)) {
          results.push(`  ✅ rtsp://${host}:${port}${path} — Status: ${r.status} (${r.status === 401 ? "auth required" : "OK"})`);
        }
      } catch {}
    }
    
    if (results.length === 1) results.push("  (no standard paths responded — try manual paths)");
    return results.join("\n");
  },

  nmap_cctv: async (target) => {
    // Run nmap with CCTV-specific scripts
    try {
      const { execSync } = await import("child_process");
      const cmd = `nmap -Pn --unprivileged -p 80,554,37777,8000,8080,8081,8554,9000,9001 --script "rtsp-methods,http-title,http-headers" ${target} 2>&1`;
      const r = execSync(cmd, { encoding: "utf-8", timeout: 60000, maxBuffer: 1024 * 1024 });
      return r.trim().substring(0, 6000);
    } catch (e) {
      return `[Nmap CCTV Error] ${e.stderr?.substring(0, 500) || e.message}`;
    }
  },
