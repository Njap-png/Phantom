...
import { BASE_DIR, MEMORY_DIR, REPORTS_DIR, PLAYBOOKS_DIR, KNOWLEDGE_DIR } from "./config.mjs";
...
export const hackerTools = {
  ...
  self_evolve: async (input) => {
    ...
    try {
      ...
      const gitPending = result.results.filter(r => r.phase === "git" && r.status === "pending_push");
      if (gitPending.length > 0) {
        for (const p of gitPending) {
          lines.push(`\n📤 ${p.count} commit(s) ready to push:`);
          lines.push(`  ${p.commits.slice(0, 300)}`);
          lines.push(`  → Type @git_push to push.`);
        }
      }
