...
import { BASE_DIR, MEMORY_DIR, REPORTS_DIR, PLAYBOOKS_DIR, KNOWLEDGE_DIR, BOOKS_DIR, PHANTOM_DIR } from "./config.mjs";
import { __r, runExternal, formatExternal, execAsync } from "./runtime.mjs";
import { get as vaultGet, set as vaultSet, remove as vaultRemove, keys as vaultKeys, status as vaultStatus, where as vaultWhere, relocate as vaultRelocate, recoverHint } from "./vault.mjs";
...
export const hackerTools = {
  shell: async (cmd) => {
    ...
    try {
      const r = await execAsync(cmd, { timeout: 30000, maxBuffer: 1024 * 1024 });
  ...
  code_gen: async (input) => {
    ...
    try {
      const code = await llm.chat([
        { role: "system", content: `Generate only ${lang} code. No explanations. No markdown.` },
        { role: "user", content: prompt }
], { model: __r._config?.default_model });
       if (code.startsWith("[") && code.includes("]")) return `[Code Gen] LLM error: ${code.substring(0, 200)}`;
      // Strip markdown code fences if present
      const clean = code.replace(/^```\w*\n?/gm, "").replace(/```$/gm, "").trim();
      if (outPath) { fs.writeFileSync(resolve(outPath), clean, "utf-8"); return `✅ Generated ${code.length} chars → ${outPath}`; }
      return code;
  ...
  llm_config: async (input) => {
    try {
      ...
      if (cmd === "set" || cmd.startsWith("set ")) {
        ...
        process.env[e] = v;
        if (/_KEY$|_TOKEN$|_SECRET$|PASSWORD/.test(e)) {
          vaultSet(e, v);
          delete __r._config[e];
        } else {
          __r._config[e] = v;
        }
        try { fs.writeFileSync(userConfigPath, JSON.stringify(__r._config, null, 2)); } catch {}
        return `✅ ${e} set and saved (secrets stored in vault)`;
      }
      if (cmd.startsWith("model ")) {
        const m = raw.slice(6).trim(); if (!m) return `[LLM] Usage: @llm_config|model name`;
  ...
  // ── SELF-EVOLVE — full pipeline: wrappers→heal→optimize→validate→test→push ──
  self_evolve_primary: async (input) => {
    ...
    try {
      ...
      const learned = result.results.filter(r => r.phase === "knowledge" && r.status === "learned");
      if (learned.length > 0) {
        lines.push(`\n📚 Absorbed ${learned.length} learned module(s):`);
        learned.forEach(l => lines.push(`  + ${l.kind}: ${l.source} → lib/learned/${l.slug}.mjs`));
      }
      ...
      if (created.length === 0 && learned.length === 0 && validated.length > 0 && failed.length === 0) {
  ...
  update: async (input) => {
    ...
    try {
      ...
      // Re-check syntax
      try { execSync(`node --check "${resolve(PHANTOM_DIR, "phantom.mjs")}"`, { stdio:"pipe", timeout:5000 }); } catch { return `[update] Pulled but SYNTAX ERROR in new code. Roll back with: git reset --hard HEAD@{1}`; }
  ...
  schedule: async (input) => {
    const SCHEDULES_FILE = resolve(homedir(), ".config", "phantom", "schedules.json");
    
    function loadSchedules() {
      if (fs.existsSync(SCHEDULES_FILE)) {
        try { return JSON.parse(fs.readFileSync(SCHEDULES_FILE, "utf-8")); } catch {}
      }
      return [];
    }
    
    function saveSchedules(schedules) {
      // Exclude sid (Timer objects) from persistence - they'll be recreated on load
      const toSave = schedules.map(s => ({ ...s, sid: undefined }));
      fs.writeFileSync(SCHEDULES_FILE, JSON.stringify(toSave, null, 2), "utf-8");
    }
    
    // Load persisted schedules on first run
    if (!globalThis.__phantomSchedulesLoaded) {
      globalThis.__phantomSchedules = loadSchedules();
      // Restore intervals
      for (const s of globalThis.__phantomSchedules) {
        s.sid = setInterval(async () => {
          const ts = new Date().toISOString();
          try {
            const result = await hackerTools[s.tool](s.target);
            console.log(`[${ts}] ⏰ ${s.tool} → ${s.target}: ${result.slice(0, 200)}`);
          } catch (e) {
            console.error(`[${ts}] ⏰ ${s.tool} → ${s.target} ERROR: ${e.message}`);
          }
        }, s.nextAt - Date.now() > 0 ? s.nextAt - Date.now() : 0);
      }
      globalThis.__phantomSchedulesLoaded = true;
    }
    
    function persistAndRestart() {
      saveSchedules(globalThis.__phantomSchedules);
...
Examples:
  ...
  subfinder: async (input) => {
    ...
    try {
      ...
      const raw = runExternal("subfinder", ["-d",domain,"-silent",...extra], {timeout:120000,maxBuffer:2*1024*1024});
  ...
  httpx: async (input) => {
    ...
    try {
      ...
      const lines = runExternal("httpx", ["-u",target,"-silent",...extra], {timeout:120000});
  ...
  nuclei: async (input) => {
    ...
    try {
      ...
      const lines = runExternal("nuclei", ["-u",target,"-silent",...extra], {timeout:180000,maxBuffer:2*1024*1024});
  ...
  dnsx: async (input) => {
    ...
    try {
      ...
      const domains = tokens.filter(t => !t.startsWith("-"));
      const target = domains[0] || "";
      const lines = runExternal("dnsx", ["-silent","-a","-resp",...flags], {input:domains.join("\n"),timeout:60000});
  ...
  config: async (input) => {
    try {
      ...
      const usbConfigPath = presolve(new URL(".", import.meta.url).pathname, "..", ".config", "phantom", "config.json");
      const systemConfigPath = presolve(BASE_DIR, "config.json");
      let cfg = {};
      for (const cfgPath of [usbConfigPath, systemConfigPath]) {
        try {
      ...
      const firstSpace = inputTrimmed.indexOf(" ");
      const cmd = firstSpace > 0 ? inputTrimmed.slice(0, firstSpace).toLowerCase() : inputTrimmed.toLowerCase();
      const rest = firstSpace > 0 ? inputTrimmed.slice(firstSpace + 1) : "";
      if (!cmd || cmd==="list"||cmd==="ls") {
        const keys = Object.keys(cfg).sort();
        if (!keys.length) return "[config] No settings saved";
        return `⚙ Config (USB: ${usbConfigPath}, System: ${systemConfigPath})\n${keys.map(k => `  ${k}: ${typeof cfg[k]==="string"&&cfg[k].length>32 ? cfg[k].slice(0,32)+"..." : JSON.stringify(cfg[k])}`).join("\n")}`;
      }
      if (cmd==="path") return `📁 USB: ${usbConfigPath}\n📁 System: ${systemConfigPath}`;
      ...
      if (cmd === "set") {
              ...
              if (/_KEY$|_TOKEN$|_SECRET$|PASSWORD/.test(upper) || ["VT_API_KEY","SHODAN_API_KEY","GITHUB_TOKEN","CHAOS_API_KEY"].includes(upper)) {
                vaultSet(upper, v);
                delete cfg[k];
                process.env[upper] = v;
                for (const cfgPath of [usbConfigPath, systemConfigPath]) {
                ...
                return `✅ ${upper} saved to the secret vault`;
              }
              cfg[k] = v;
              for (const cfgPath of [usbConfigPath, systemConfigPath]) {
                try { fs.writeFileSync(cfgPath, JSON.stringify(cfg,null,2)); } catch {}
              ...
              if (upper.endsWith("_API_KEY")||["VT_API_KEY","SHODAN_API_KEY","GITHUB_TOKEN","HACKERONE_API_TOKEN","HACKERONE_API_USERNAME","BUGCROWD_API_TOKEN","BUGCROWD_API_USERNAME","CHAOS_API_KEY"].includes(upper)) process.env[upper] = v;
...
╚${"═".repeat(36)}╝`;
  ...
  evolve: async (input) => {
    ...
    try {
      ...
      if (action === "validate") {
        const scanDirs = [
          ...fs.readdirSync(phantomDir).filter(f => f.endsWith(".mjs") || f.endsWith(".js")).map(f => f),
          ...fs.readdirSync(resolve(phantomDir, "lib")).filter(f => f.endsWith(".mjs")).map(f => `lib/${f}`),
          ...fs.readdirSync(resolve(phantomDir, "test")).filter(f => f.endsWith(".mjs")).map(f => `test/${f}`),
        ];
        // Also scan generated subdirs if they exist
        for (const sub of ["generated", "auto_tools", "learned"]) {
          const dir = resolve(phantomDir, "lib", sub);
          if (fs.existsSync(dir)) {
            scanDirs.push(...fs.readdirSync(dir).filter(f => f.endsWith(".mjs")).map(f => `lib/${sub}/${f}`));
          }
        }
        const results = scanDirs.map(file => {
          try {
        ...
        if (failed.length) {
          ...
          try {
            ...
            const fixed = healed.filter(h => h.fixed).length;
            if (fixed > 0) {
              output.push(`✅ Auto-repaired ${fixed} file(s)`);
              // Re-check
              const recheck = failed.map(f => f.file).filter(f => {
                try { execSync(`node --check "${resolve(phantomDir, f)}"`, { encoding: "utf-8", timeout: 10000 }); return true; }
                catch { return false; }
              });
              if (recheck.length) output.push(`✅ ${recheck.length} previously failed now pass`);
              const stillFailing = failed.filter(f => !recheck.includes(f.file));
              if (stillFailing.length) output.push(`❌ ${stillFailing.length} still failing — fix manually with @self_edit`);
            } else {
              output.push(`⚠ Auto-repair unable to fix — check files manually`);
            }
          } catch { output.push(`⚠ Auto-repair unavailable`); }
  ...
  learn_from: async (input) => {
    try {
      const { cloneOrLocate, scanProject, extractPatterns, savePatterns, extractTechniques } = await import("./self_improve.mjs");
      const args = input.trim();
      if (!args) {
        return `[learn_from] Usage: @learn_from|<git_url|local_path>
  Analyzes a project's structure, patterns, and techniques without generating code.
  Examples:
    ...
    @learn_from|/path/to/project`;
      ...
      // New: extract payloads/techniques from markdown files
      const techResult = extractTechniques(project.path, project.name);
      const techLines = [];
      if (techResult.totalTechniques > 0) {
        techLines.push(``, `🧪 Techniques Extracted:`);
        techLines.push(`  Markdown files: ${techResult.totalFiles}`);
        techLines.push(`  Total techniques/payloads: ${techResult.totalTechniques}`);
        techLines.push(`  Categories:`);
        for (const [cat, count] of Object.entries(techResult.categories).sort((a, b) => b[1] - a[1]).slice(0, 20)) {
          techLines.push(`    • ${cat}: ${count} techniques`);
        }
        if (Object.keys(techResult.categories).length > 20) {
          techLines.push(`    ... and ${Object.keys(techResult.categories).length - 20} more categories`);
        }
        techLines.push(`  Technique books saved: ${techResult.bookFiles.length}`);
      }
      ...
      // ── Auto-evolve: generate tools + commit + push ──
      try {
        const { autoEvolve } = await import("./evolve.mjs");
        const evolved = await autoEvolve();
        const evolveLines = [``, `🧬 Auto-Evolution:`];
        for (const r of evolved.results) {
          if (r.phase === "wrapper" && r.status === "created") {
            evolveLines.push(`  ✓ Generated tool: ${r.bin}`);
          } else if (r.phase === "git" && r.status === "committed") {
            evolveLines.push(`  ✓ Committed ${r.files} file(s)`);
          } else if (r.phase === "git" && r.status === "pushed") {
            evolveLines.push(`  ✓ Pushed to remote`);
          } else if (r.phase === "git" && r.status === "blocked") {
            evolveLines.push(`  ⚠ Push blocked: ${r.reason || ""}`);
          } else if (r.phase === "git" && r.status === "push_failed") {
            evolveLines.push(`  ⚠ Push failed: ${r.reason || ""}`);
          } else if (r.phase === "test" && r.status === "passed") {
            evolveLines.push(`  ✓ Tests passed`);
          } else if (r.phase === "test" && r.status === "failed") {
            evolveLines.push(`  ✗ Tests failed`);
          }
        }
        if (evolveLines.length > 1) lines.push(...evolveLines);

        // If no wrappers generated but techniques learned, still commit knowledge changes
        if (techResult.totalTechniques > 0 && !evolved.results.some(r => r.phase === "git" && r.status === "committed")) {
          const { gitCommit, gitPush } = await import("./evolve.mjs");
          const committed = gitCommit([], `🧬 learn: ${techResult.totalTechniques} techniques from ${project.name}`);
          if (committed) {
            lines.push(`  ✓ Knowledge committed`);
            const pResult = gitPush();
            if (pResult.pushed) lines.push(`  ✓ Pushed to remote`);
          }
        }
      } catch (e) {
        lines.push(``, `  ⚠ Auto-evolve skipped: ${e.message}`);
      }
  ...
  apply_improvements: async (input) => {
    try {
      ...
      if (args === "all") {
        const { execSync } = await import("child_process");
        const learnedDir = resolve(PHANTOM_DIR, "lib", "learned");
  ...
  // ── SELF-EVOLVE (alias) — route to primary handler ────────
  self_evolve: async (input) => hackerTools.self_evolve_primary(input),

  // ── SELF-HEAL — full auto-diagnose → fix → test → verify loop ──
  self_heal: async (input) => {
    try {
      const { selfHeal } = await import("./evolve.mjs");
      const opts = {};
      if (input?.trim()) {
        const n = parseInt(input.trim());
        if (!isNaN(n)) opts.maxAttempts = n;
      }
      const result = await selfHeal(opts);
      let out = `🧬 Self-Heal completed in ${(result.elapsed / 1000).toFixed(1)}s\n`;
      out += `Status: ${result.clean ? '✅ Clean' : '⚠ Needs attention'}\n`;
      out += `Rounds: ${result.attempts}\n\n`;
      for (const round of result.log) {
        out += `Round ${round.attempt}: `;
        const errors = round.checks.filter(c => c.status === 'error').length;
        const fixed = round.fixes.filter(f => f.fixed).length;
        const tests = round.tests.map(t => t.status).join(', ');
        out += `${errors} errors, ${fixed} fixes, tests: ${tests || 'pending'}\n`;
        if (round.fixes.length > 0) {
          for (const f of round.fixes) {
            out += `  ${f.fixed ? '✅' : '❌'} ${f.file}: ${f.error}\n`;
          }
        }
        if (round.push) out += `  📤 Push: ${round.push}\n`;
      }
      return out;
    } catch (e) { return `[self_heal Error] ${e.message}`; }
  },
  ...
  git_sync: async (input) => {
    try {
      ...
      if (cmd === "push") {
        // Apply git credentials from config if available
        try {
          const { resolve:presolve } = await import("path");
          const usbConfigPath = presolve(new URL(".", import.meta.url).pathname, "..", "config.json");
          const systemConfigPath = presolve(BASE_DIR, "config.json");
          ...
          let projectConfig = {};
          for (const cfgPath of [usbConfigPath]) {
            try {
              if (fs.existsSync(cfgPath)) {
                projectConfig = JSON.parse(fs.readFileSync(cfgPath, "utf-8"));
                break;
              }
            } catch {}
          }
          ...
          let userConfig = {};
          for (const cfgPath of [systemConfigPath]) {
            try {
              if (fs.existsSync(cfgPath)) {
          ...
          let cfg = deepMerge(projectConfig, userConfig);
          // Auth is handled by the global credential helper (~/.config/phantom/git-credential-helper.sh),
          // so we never embed GIT_TOKEN into the remote URL (keeps it out of .git/config).
          if (cfg.GIT_EMAIL) run(`git config user.email "${cfg.GIT_EMAIL}" 2>/dev/null`);
          if (cfg.GIT_USER) run(`git config user.name "${cfg.GIT_USER}" 2>/dev/null`);
        } catch {}
        ...
        // 4. Alignment check before staging
        try {
          const { verifyDiffAlignment } = await import("./evolve.mjs");
          const root = run("git rev-parse --show-toplevel").trim();
          const alignment = verifyDiffAlignment(root);
          if (!alignment.ok) {
            return `⛔ Push blocked — alignment check failed:\n  - ${alignment.errors.join("\n  - ")}`;
          }
          if (alignment.warnings.length > 0) {
            // Just warn, don't block
          }
        } catch { /* alignment check unavailable */ }
...
`;
  ...
  // ── VAULT — secret storage phantom alone manages ──
  vault: async (input) => {
    try {
      const parts = (input || "").trim().split(/\s+/).filter(Boolean);
      const cmd = (parts[0] || "status").toLowerCase();
      const rest = parts.slice(1);
      const USAGE = `🔒 Secret vault\nCommands:\n  status                   — where it is + what's stored\n  where                     — current vault file path\n  list                      — stored key names (never values)\n  move                      — relocate to a fresh hidden dir\n  set <key> <value>         — save a secret\n  get <key>                 — confirm a key is stored\n  remove <key>              — delete a secret`;

      if (cmd === "where" || cmd === "location") {
        return `🔒 ${vaultStatus().location}`;
      }
      if (cmd === "list" || cmd === "keys") {
        const ks = vaultKeys();
        return `🔒 Vault keys (${ks.length}): ${ks.join(", ") || "none"}`;
      }
      if (cmd === "move" || cmd === "relocate") {
        const before = vaultWhere();
        const after = vaultRelocate();
        return `🔒 Vault relocated ${before} → ${after.vault}\n  New location recorded in memory. Total moves: ${after.moves}.`;
      }
      if (cmd === "set") {
        if (rest.length < 2) return `[vault] Usage: @vault|set KEY value`;
        const k = rest[0].toUpperCase();
        const v = rest.slice(1).join(" ");
        vaultSet(k, v);
        return `✅ ${k} saved to the secret vault`;
      }
      if (cmd === "get") {
        if (rest.length < 1) return `[vault] Usage: @vault|get KEY`;
        const k = rest[0].toUpperCase();
        const v = vaultGet(k);
        if (v === undefined) return `🔒 ${k} not in vault`;
        return `🔒 ${k} stored (${String(v).length} chars) — value accessible to phantom internally`;
      }
      if (cmd === "remove") {
        if (rest.length < 1) return `[vault] Usage: @vault|remove KEY`;
        const k = rest[0].toUpperCase();
        return vaultRemove(k) ? `🗑 ${k} removed` : `🔒 ${k} not in vault`;
      }
      if (cmd === "recover") {
        const hints = recoverHint();
        return `🔒 Candidate vault dirs: ${hints.join(", ") || "none"}`;
      }
      const s = vaultStatus();
      return `🔒 Secret vault\n  Location: ${s.location}\n  Pointer: ${s.pointer}\n  Moves: ${s.moves} · Last move: ${s.updated}\n  Keys: ${s.keys.join(", ") || "none"}\n\n${USAGE}`;
    } catch (e) { return `[vault Error] ${e.message}`; }
  },

  // ── GITHUB — credentials for unattended auto-push ──
  github_setup: async (input) => {
    try {
      const parts = (input || "").trim().split(/\s+/).filter(Boolean);
      const cmd = (parts[0] || "help").toLowerCase();
      const { isInteractive } = await import("./credentials.mjs");
      const { gitRoot, gitPush } = await import("./evolve.mjs");
      const { execSync } = await import("child_process");

      if (cmd === "status") {
        const stored = !!vaultGet("GITHUB_TOKEN") || !!process.env.GITHUB_TOKEN;
        const root = gitRoot();
        let remote = "none";
        if (root) {
          try { remote = execSync("git remote get-url origin", { cwd: root, encoding: "utf-8", timeout: 5000 }).trim(); } catch {}
        }
        return `🐙 GitHub auto-push\n  Token: ${stored ? "stored in vault ✓" : "not set — auto-push is off"}\n  Repo:  ${root || "not a git checkout"}\n  Remote: ${remote || "no origin"}\n\n${stored ? "Pushes run unattended and never prompt." : "Run @github_setup|set to enable. Needs a token with write access to this repo."}`;
      }

      if (cmd === "set") {
        if (!isInteractive()) return "[github_setup] Cannot prompt in non-interactive mode. Set GITHUB_TOKEN in the environment, or use @vault|set GITHUB_TOKEN <value>.";
        // A token pasted inline would land in the transcript; hide the input.
        const { askHidden } = await import("./credentials.mjs");
        const token = (await askHidden("🔑 GitHub token (hidden): ") || "").trim();
        if (!token) return "[github_setup] No token entered — unchanged.";
        if (!/^gh[pousr]_/.test(token)) return "[github_setup] That doesn't look like a GitHub token (expected a ghp_/gho_/ghu_/ghs_/ghr_ prefix). Nothing saved.";
        vaultSet("GITHUB_TOKEN", token);
        process.env.GITHUB_TOKEN = token;
        return `✅ Token saved to the secret vault (${token.length} chars).\n  It is stored in the vault only — never in config.json, .git/config or a remote URL.\n  Next auto-push will use it. Test it with @github_setup|test.`;
      }

      if (cmd === "remove" || cmd === "clear") {
        const had = vaultRemove("GITHUB_TOKEN");
        delete process.env.GITHUB_TOKEN;
        return had ? "🗑 GITHUB_TOKEN removed from the vault — auto-push is off" : "🔒 No GITHUB_TOKEN in the vault";
      }

      if (cmd === "test") {
        const r = gitPush();
        return r.pushed
          ? "✅ Pushed successfully — credentials work."
          : `✗ Push not completed: ${r.reason}`;
      }

      return "🐙 GitHub auto-push\n  Commands:\n  status    — token + remote state\n  set       — enter a token (hidden input)\n  test      — attempt a push now\n  remove    — delete the stored token";
    } catch (e) { return `[github_setup Error] ${e.message}`; }
  },
  ...
  // ── HACKERONE BUG BOUNTY TOOL ──
  hackerone: async (input) => {
    try {
      const { resolve:presolve } = await import("path");
      const usbConfigPath = presolve(new URL(".", import.meta.url).pathname, "..", "config.json");
      const systemConfigPath = presolve(BASE_DIR, "config.json");
      ...
      const VALID_COMMANDS = new Set(["programs", "scope", "reports", "report", "submit", "me", "test", "help", "list"]);
      
      // Help/list work without API key
      if (!cmd || cmd === "help" || cmd === "list") {
        return `🏴‍☠️ HackerOne Tool
Usage: @hackerone <command> [args]

Commands:
  programs                    — List your programs
  scope <handle>              — Get scope (in/out of scope) for a program
  reports <handle> [state]    — List reports (states: new, triaged, resolved, all)
  report <id>                 — Get full report details
  submit <handle> <title> <severity> <details> — Submit draft report
  me                          — Show authenticated user info
  test                        — Test authentication (read-only)

Examples:
  @hackerone programs
  @hackerone scope acme-corp
  @hackerone reports acme-corp triaged
  @hackerone report 1234567
  @hackerone submit acme-corp "XSS in login" high "Reflected XSS at /login?next=..."
  @hackerone test`;
      }
      
      // Get credentials from env or vault/config
      let username = process.env.HACKERONE_API_USERNAME || vaultGet("HACKERONE_API_USERNAME") || cfg.HACKERONE_API_USERNAME || cfg.hackerone_api_username;
      let token = process.env.HACKERONE_API_TOKEN || vaultGet("HACKERONE_API_TOKEN") || cfg.HACKERONE_API_TOKEN || cfg.hackerone_api_token;

      if (cmd && !VALID_COMMANDS.has(cmd)) {
        return `[hackerone] Unknown command: ${cmd}. Use @hackerone help`;
      }

      // If a task needs credentials and they are missing, ask for them (TTY only)
      if (!username || !token) {
        const { ensure } = await import("./credentials.mjs");
        username = username || await ensure("HACKERONE_API_USERNAME");
        token = token || await ensure("HACKERONE_API_TOKEN");
      }

      if (!username || !token) {
        return `[hackerone] Set HACKERONE_API_USERNAME and HACKERONE_API_TOKEN
Usage:
  @config|set HACKERONE_API_USERNAME your_token_identifier
  @config|set HACKERONE_API_TOKEN your_token_value
  @vault|set HACKERONE_API_USERNAME your_token_identifier
  @vault|set HACKERONE_API_TOKEN your_token_value
Or set env vars HACKERONE_API_USERNAME and HACKERONE_API_TOKEN`;
      }
      ...
      async function h1Fetch(path, options = {}) {
        const r = await fetch(`${base}${path}`, { headers, signal: AbortSignal.timeout(20000), ...options });
        if (!r.ok) {
      ...
      if (cmd === "test") {
        try {
          const d = await h1Fetch("/hackers/me/reports");
          return `[hackerone] Auth test: HTTP 200 OK — authenticated successfully`;
        } catch (e) {
          return `[hackerone] Auth test failed: ${e.message}`;
        }
      }
      
      if (cmd === "me") {
        try {
          const d = await h1Fetch("/hackers/me");
          const u = d.data?.attributes || {};
          return `👤 ${u.username} (${u.name || "N/A"}) — ${u.profile_url || "https://hackerone.com/" + (u.username || "")}\nSignal: ${u.signal || "?"} | Reputation: ${u.reputation || "?"} | Impact: ${u.impact || "?"}`;
        } catch (e) {
          // Fallback: try to get info from reports endpoint
          try {
            const d = await h1Fetch("/hackers/me/reports?page[size]=1");
            return `👤 Authenticated (token works for reports)\nFull profile access may require additional permissions.`;
          } catch {
            return `[hackerone] Auth works for reports but /hackers/me requires additional permissions: ${e.message}`;
          }
        }
      }
      
      if (cmd === "programs") {
        const d = await h1Fetch("/hackers/programs");
        const progs = d.data || [];
        if (!progs.length) return "[hackerone] No programs found";
        return progs.map(p => {
          const a = p.attributes || {};
          return `  ${a.handle} — ${a.name} (${a.state}) | ${a.offers_bounties ? "💰" : "🏅"} | https://hackerone.com/${a.handle}`;
        }).join("\n");
      }
      
      if (cmd === "scope") {
        const handle = args[0];
        if (!handle) return "[hackerone] Usage: @hackerone scope <program_handle>";
        const d = await h1Fetch(`/hackers/programs/${handle}/structured_scopes`);
        const scopes = d.data || [];
        if (!scopes.length) return `[hackerone] No structured scopes for ${handle}`;
        const inScope = scopes.filter(s => s.attributes?.eligible_for_bounty === true && s.attributes?.eligible_for_submission === true);
        const outScope = scopes.filter(s => s.attributes?.eligible_for_bounty === false || s.attributes?.eligible_for_submission === false);
        let out = `🎯 Scope for ${handle}\n`;
        if (inScope.length) {
          out += "\n✅ IN SCOPE (bounty eligible):\n" + inScope.map(s => {
            const a = s.attributes || {};
            return `  ${a.asset_identifier} (${a.asset_type}) — ${a.instruction || "No instructions"} [max severity: ${a.max_severity || "?"}]`;
          }).join("\n");
        }
        if (outScope.length) {
          out += "\n❌ OUT OF SCOPE:\n" + outScope.map(s => {
            const a = s.attributes || {};
            return `  ${a.asset_identifier} (${a.asset_type}) — ${a.instruction || "Not eligible"}`;
          }).join("\n");
        }
        return out;
      }
      ...
      if (cmd === "reports") {
        ...
        if (!handle) return "[hackerone] Usage: @hackerone reports <program_handle> [state]";
        const stateParam = state === "all" ? "" : `&filter[state]=${state}`;
        const d = await h1Fetch(`/hackers/programs/${handle}/reports?page[size]=20${stateParam}`);
        const reports = d.data || [];
        if (!reports.length) return `[hackerone] No reports for ${handle} (state: ${state})`;
        return reports.map(r => {
          const a = r.attributes || {};
          return `  #${r.id} — ${a.title} | ${a.severity_rating || a.severity || "?"} | ${a.state} | ${a.created_at}`;
        }).join("\n");
      }
      
      if (cmd === "report") {
        const id = args[0];
        if (!id) return "[hackerone] Usage: @hackerone report <report_id>";
        const d = await h1Fetch(`/reports/${id}`);
        const r = d.data?.attributes || {};
        const rel = d.included || [];
        const program = rel.find(x => x.type === "program")?.attributes?.handle || "?";
        return `📋 Report #${id} (${program})
Title: ${r.title}
State: ${r.state} | Severity: ${r.severity_rating || r.severity || "?"}
Assigned: ${r.assignee?.attributes?.username || "unassigned"}
Created: ${r.created_at}
Updated: ${r.updated_at}
Weakness: ${r.weakness?.name || "N/A"}

${r.vulnerability_information ? r.vulnerability_information.substring(0, 2000) : "(no details)"}`;
      }
      ...
      if (cmd === "submit") {
        ...
        if (!handle || !title || !severity || !details) {
          return "[hackerone] Usage: @hackerone submit <handle> <title> <severity> <details>\nSeverity: critical, high, medium, low, none";
        }
        const body = JSON.stringify({
          data: {
            ...
            attributes: {
              ...
              severity_rating: severity.toLowerCase(),
              program: { data: { type: "program", attributes: { handle } } }
            }
          }
        });
        const d = await h1Fetch("/reports", { method: "POST", headers: { ...headers, "Content-Type": "application/json" }, body });
        return `✅ Report submitted: #${d.data?.id} — ${d.data?.attributes?.title} (${d.data?.attributes?.state})`;
      }
      
      return `[hackerone] Unknown command: ${cmd}. Use @hackerone help`;
    } catch (e) {
      return `[hackerone Error] ${e.message}`;
    }
  },

  // ── BUGCROWD BUG BOUNTY TOOL ──
  bugcrowd: async (input) => {
    try {
      const { resolve:presolve } = await import("path");
      const usbConfigPath = presolve(new URL(".", import.meta.url).pathname, "..", "config.json");
      const systemConfigPath = presolve(BASE_DIR, "config.json");
      ...
      if (!cmd || cmd === "help" || cmd === "list") {
        return `🐛 Bugcrowd Tool
Usage: @bugcrowd <command> [args]

Commands:
  programs                    — List your programs
  scope <program_uuid>        — Get scope (targets) for a program
  submissions <program_uuid>  — List submissions (states: new, triaged, resolved, all)
  submission <id>             — Get full submission details
  me                          — Show authenticated user info
  test                        — Test authentication (read-only)

Examples:
  @bugcrowd programs
  @bugcrowd scope 12345678-1234-1234-1234-123456789012
  @bugcrowd submissions 12345678-1234-1234-1234-123456789012 triaged
  @bugcrowd submission 987654321
  @bugcrowd test`;
      }
      
      const apiToken = process.env.BUGCROWD_API_TOKEN || cfg.BUGCROWD_API_TOKEN || cfg.bugcrowd_api_token;
      const username = process.env.BUGCROWD_API_USERNAME || cfg.BUGCROWD_API_USERNAME || cfg.bugcrowd_api_username || "token";
      
      if (!apiToken) {
        return `[bugcrowd] Set BUGCROWD_API_TOKEN
Usage:
  @config|set BUGCROWD_API_TOKEN your_api_token
Or set env var BUGCROWD_API_TOKEN`;
      }
      
      const base = "https://bugcrowd.com/h/api/v1";
      const auth = "Token " + apiToken;
      const headers = { "Authorization": auth, "Accept": "application/json", "User-Agent": "Phantom/0.2.0" };
      ...
      async function bcFetch(path, options = {}) {
        ...
        if (!contentType.includes("application/json")) {
          ...
          if (text.includes("<!DOCTYPE") || text.includes("<html")) {
            throw new Error(`API returned HTML instead of JSON. Bugcrowd API requires OAuth2/Okta authentication. Check if endpoint ${base}${path} is correct or if you need a different auth method.`);
          }
      ...
      if (cmd === "test") {
        try {
          const d = await bcFetch("/user");
          return `[bugcrowd] Auth test: HTTP 200 OK — authenticated as ${d.username || d.email || "user"}`;
        } catch (e) {
          return `[bugcrowd] Auth test failed: ${e.message}`;
        }
      }
      ...
      if (cmd === "me") {
        try {
          const d = await bcFetch("/user");
          return `👤 ${d.username} (${d.email || "N/A"}) — ${d.name || "N/A"}\nResearcher since: ${d.created_at || "?"}`;
        } catch (e) {
          return `[bugcrowd] Error: ${e.message}`;
        }
      }
      
      if (cmd === "programs") {
        const d = await bcFetch("/programs");
        const progs = d.programs || d.data || [];
        if (!progs.length) return "[bugcrowd] No programs found";
        return progs.map(p => {
          return `  ${p.name} (${p.code}) — ${p.description?.substring(0, 60) || "No description"} | State: ${p.state} | ${p.url}`;
        }).join("\n");
      }
      
      if (cmd === "scope") {
        const programUuid = args[0];
        if (!programUuid) return "[bugcrowd] Usage: @bugcrowd scope <program_uuid>";
        const d = await bcFetch(`/programs/${programUuid}/targets`);
        const targets = d.targets || d.data || [];
        if (!targets.length) return `[bugcrowd] No targets for ${programUuid}`;
        const inScope = targets.filter(t => t.in_scope === true);
        const outScope = targets.filter(t => t.in_scope === false);
        let out = `🎯 Scope for ${programUuid}\n`;
        if (inScope.length) {
          out += "\n✅ IN SCOPE:\n" + inScope.map(t => {
            return `  ${t.name} (${t.category}) — ${t.instructions || "No instructions"} [reward: ${t.max_payout || "?"}]`;
          }).join("\n");
        }
        if (outScope.length) {
          out += "\n❌ OUT OF SCOPE:\n" + outScope.map(t => {
            return `  ${t.name} (${t.category}) — ${t.instructions || "Not eligible"}`;
          }).join("\n");
        }
        return out;
      }
      ...
      if (cmd === "submissions") {
        const programUuid = args[0];
        const state = args[1] || "all";
        if (!programUuid) return "[bugcrowd] Usage: @bugcrowd submissions <program_uuid> [state]";
        const stateParam = state === "all" ? "" : `&state=${state}`;
        const d = await bcFetch(`/programs/${programUuid}/submissions?per_page=20${stateParam}`);
        const subs = d.submissions || d.data || [];
        if (!subs.length) return `[bugcrowd] No submissions for ${programUuid} (state: ${state})`;
        return subs.map(s => {
          return `  #${s.id} — ${s.title} | ${s.vrt?.substring(0, 40) || "?"} | ${s.state} | ${s.created_at}`;
        }).join("\n");
      }
      
      if (cmd === "submission") {
        const id = args[0];
        if (!id) return "[bugcrowd] Usage: @bugcrowd submission <submission_id>";
        const d = await bcFetch(`/submissions/${id}`);
        const s = d.submission || d.data || {};
        return `📋 Submission #${id}\nTitle: ${s.title}\nState: ${s.state} | Priority: ${s.priority || "?"}\nVRT: ${s.vrt || "N/A"}\nProgram: ${s.program?.name || "?"}\nCreated: ${s.created_at}\n\n${s.description?.substring(0, 2000) || "(no details)"}`;
      }
      
      return `[bugcrowd] Unknown command: ${cmd}. Use @bugcrowd help`;
    } catch (e) {
      return `[bugcrowd Error] ${e.message}`;
    }
  },

  // ── UNIFIED BUG BOUNTY TOOL (HackerOne + Bugcrowd) ──
  bugbounty: async (input) => {
    try {
      const parts = (input || "").trim().split(/\s+/);
      ...
      if (!platform || platform === "help" || platform === "list") {
        return `🏴‍☠️ Bug Bounty Platforms (Unified)
Usage: @bugbounty <platform> <command> [args]

Platforms:
  hackerone, h1     — HackerOne
  bugcrowd, bc      — Bugcrowd

Examples:
  @bugbounty hackerone programs
  @bugbounty h1 scope acme-corp
  @bugbounty bugcrowd programs
  @bugcrowd bc scope 12345678-1234-1234-1234-123456789012
  @bugbounty hackerone reports acme-corp triaged
  @bugbounty bugcrowd submissions 12345678-1234-1234-1234-123456789012 new

Commands per platform:
  hackerone: programs, scope, reports, report, submit, me, test
  bugcrowd:  programs, scope, submissions, submission, me, test

Config (in config.json or env):
  HackerOne: HACKERONE_API_USERNAME + HACKERONE_API_TOKEN
  Bugcrowd:  BUGCROWD_API_TOKEN (+ optional BUGCROWD_API_USERNAME)`;
      }
      
      if (platform === "hackerone" || platform === "h1") {
        const { hackerTools } = await import("./tools.mjs");
        return hackerTools.hackerone(`${cmd} ${args}`.trim());
      }
      
      if (platform === "bugcrowd" || platform === "bc") {
        const { hackerTools } = await import("./tools.mjs");
        return hackerTools.bugcrowd(`${cmd} ${args}`.trim());
      }
      
      return `[bugbounty] Unknown platform: ${platform}. Use: hackerone, bugcrowd`;
    } catch (e) {
      return `[bugbounty Error] ${e.message}`;
    }
  },

  // ── AUTONOMOUS PASSIVE MISSION ORCHESTRATION ──
  mission: async (input) => {
    try {
      const parts = (input || "").trim().split(/\s+/);
      const cmd = parts[0]?.toLowerCase();
      const missionArg = parts[1];

      if (!cmd || cmd === "help" || cmd === "list") {
        const { listMissions } = await import("./mission.mjs");
        const missions = listMissions();
        if (missions.length === 0) {
          return `🎯 Mission Orchestration\nUsage: @mission <command> [args]\n\nCommands:\n  auto <program_handle>   — Create mission from HackerOne program & run passive recon\n  list                    — List existing missions\n  status <mission_id>     — Show mission status\n  summary <mission_id>    — Generate final report\n  pause <mission_id>      — Pause running mission\n  resume <mission_id>     — Resume paused mission\n  stop <mission_id>       — Gracefully stop (allows resume)\n  cancel <mission_id>     — Cancel mission permanently\n\nExample:\n  @mission auto cloudflare`;
        }
        return `🎯 Mission Orchestration\nUsage: @mission <command> [args]\n\nCommands:\n  auto <program_handle>   — Create mission from HackerOne program & run passive recon\n  list                    — List existing missions\n  status <mission_id>     — Show mission status\n  summary <mission_id>    — Generate final report\n  pause <mission_id>      — Pause running mission\n  resume <mission_id>     — Resume paused mission\n  stop <mission_id>       — Gracefully stop (allows resume)\n  cancel <mission_id>     — Cancel mission permanently\n\nExisting missions:\n${missions.map(m => `  ${m.id} — ${m.programName} (${m.programHandle}) [${m.status}]`).join("\n")}\n\nExample:\n  @mission auto cloudflare`;
      }
      ...
      if (cmd === "list") {
        ...
        return missions.map(m => `  ${m.id} — ${m.programName} (${m.programHandle}) [${m.status}]`).join("\n");
      }

      if (!missionArg) {
        return `[mission] Usage: @mission <command> <mission_id|program_handle>`;
      }

      if (cmd === "auto") {
        // Check if missionArg is a mission ID (starts with "mission-") or a program handle
        const isMissionId = missionArg.startsWith("mission-");
        
        if (isMissionId) {
          // Load existing mission
          const { loadMission } = await import("./mission.mjs");
          const { generateReconPlan } = await import("./mission.mjs");
          const { setExecutionState, EXEC_STATES, startRecon } = await import("./recon.mjs");
          
          const mission = loadMission(missionArg);
          if (!mission) {
            return `[mission] Mission not found: ${missionArg}`;
          }
          
          // Verify valid HackerOne scope using existing Stage 1 scope guard
          if (!mission.scope || !mission.scope.inScope || mission.scope.inScope.length === 0) {
            return `[mission] No in-scope assets found for ${missionArg}. Cannot create auto plan.`;
          }
          
          // Create execution plan using existing recon planner
          const plan = generateReconPlan(mission);
          
          // Persist "auto" execution state for resume/pause/stop support
          setExecutionState(missionArg, EXEC_STATES.READY);
          
          // Update mission status to "auto-planned"
          const { updateMissionStatus } = await import("./mission.mjs");
          updateMissionStatus(missionArg, "auto-planned");
          
          // Execute passive recon using existing executor (phases 1-3)
          // This reuses lib/recon.mjs completely - no duplicated logic
          const findings = await startRecon(missionArg);
          
          // Generate summary
          const { generateReconSummary, getExecutionState: getState } = await import("./recon.mjs");
          const summary = generateReconSummary(missionArg);
          const finalState = getState(missionArg);
          if (finalState === EXEC_STATES.FAILED) {
            return `⚠️ Mission ${missionArg} stopped: no progress for ${process.env.PHANTOM_IDLE_TIMEOUT_MS || "120000"}ms (idle/hang detected).\nIn-scope targets: ${mission.scope.inScope.length}\nResume with: @mission resume ${missionArg}\n\n${summary}`;
          }
          return `✅ Mission ${missionArg} auto-planned and passive recon completed for ${mission.programName}\nIn-scope targets: ${mission.scope.inScope.length}\n\n${summary}`;
        }
        
        // Original behavior: create mission from HackerOne program handle
        const programHandle = missionArg;
        const { fetchProgramAndScope, createMission, saveMission } = await import("./mission.mjs");
        const { startRecon } = await import("./recon.mjs");

        // Fetch program and scope from HackerOne
        const { program, scope } = await fetchProgramAndScope(programHandle);

        if (scope.inScope.length === 0) {
          return `[mission] No in-scope assets found for ${programHandle}. Cannot create mission.`;
        }

        // Create mission
        const mission = createMission(program, scope);
        saveMission(mission);

        // Run passive recon (phases 1-3)
        const findings = await startRecon(mission.id);

        // Generate summary
        const { generateReconSummary } = await import("./recon.mjs");
        const summary = generateReconSummary(mission.id);

        return `✅ Mission ${mission.id} created and passive recon completed for ${program.name}\nIn-scope targets: ${scope.inScope.length}\n\n${summary}`;
      }

      if (cmd === "status") {
        const { getExecutionState } = await import("./recon.mjs");
        const state = getExecutionState(missionArg);
        return state ? `📊 ${missionArg}: ${state}` : `[mission] No execution state for ${missionArg}`;
      }

      if (cmd === "summary") {
        const { generateReconSummary } = await import("./recon.mjs");
        return generateReconSummary(missionArg) || `[mission] Mission not found: ${missionArg}`;
      }

      if (cmd === "pause") {
        const { pauseRecon } = await import("./recon.mjs");
        return pauseRecon(missionArg);
      }

      if (cmd === "resume") {
        const { resumeRecon } = await import("./recon.mjs");
        return await resumeRecon(missionArg);
      }

      if (cmd === "stop") {
        const { stopRecon } = await import("./recon.mjs");
        return stopRecon(missionArg);
      }

      if (cmd === "cancel") {
        const { cancelMission } = await import("./recon.mjs");
        return cancelMission(missionArg);
      }

      if (cmd === "execute-next") {
        const { getNextAction } = await import("./recon-planner.mjs");
        const { isInScope, normalizeTarget, loadMission } = await import("./mission.mjs");
        const { getExecutionState, setExecutionState, EXEC_STATES, transitionState, appendActivity, ensureMissionDirs, saveResult, getResultKey, resultExists, loadExistingResult, saveError } = await import("./recon.mjs");
        const { updateMissionStatus } = await import("./mission.mjs");
        ...
        // Check execution state - must be in reconnaissance or ready to continue
        const currentState = getExecutionState(missionArg);
        if (currentState === EXEC_STATES.PAUSED || currentState === EXEC_STATES.FAILED || currentState === EXEC_STATES.COMPLETED || currentState === EXEC_STATES.CANCELLED) {
          return `[mission] Cannot execute-next: mission is ${currentState}. Use resume or create new mission.`;
        }

        // Initialize execution state for a fresh mission
        if (!currentState) {
          setExecutionState(missionArg, EXEC_STATES.READY);
        }

        // Generate the next deterministic passive action using the planner
        const nextActionResult = getNextAction(missionArg);
        if (!nextActionResult.action) {
          return `[mission] No valid actions available: ${nextActionResult.reason}`;
        }

        const action = nextActionResult.action;
        const { target, tool, phase, technique } = action;

        // Stage 1 scope re-check - immediately re-verify target is in scope
        const missionObj = loadMission(missionArg);
        const scopeCheck = isInScope(target, missionObj);
        if (!scopeCheck.allowed) {
          // Record denial and do NOT contact target
          appendActivity(missionArg, {
            action: "scope_denied",
            phase,
            tool,
            target,
          ...
          return `[mission] Target denied by Stage 1 scope guard: ${target} — ${scopeCheck.reason}. No action taken.`;
        }

        // Check for duplicate - has this action already been completed?
        const resultKey = getResultKey(phase, tool, target);
        if (resultExists(missionArg, resultKey)) {
          // Skip duplicate, try next action? No - per spec: do not select replacement automatically
          appendActivity(missionArg, {
          ...
          return `[mission] Duplicate action skipped: ${tool} on ${target} (phase ${phase}) already completed.`;
        }

        // Ensure mission dirs exist
        ensureMissionDirs(missionArg);

        // Transition to reconnaissance if not already
        if (currentState !== EXEC_STATES.RECONNAISSANCE) {
          transitionState(missionArg, EXEC_STATES.RECONNAISSANCE);
          updateMissionStatus(missionArg, "reconnaissance");
          appendActivity(missionArg, { action: "recon_start", phase });
        }

        // Execute ONLY the existing passive recon tool/action
        // Map tool to actual function from the hackerTools registry
        let toolFn;
        switch (tool) {
          case "subfinder": toolFn = hackerTools.subfinder; break;
          case "dnsx": toolFn = hackerTools.dnsx; break;
          case "httpx": toolFn = hackerTools.httpx; break;
          case "nuclei": toolFn = hackerTools.nuclei; break;
          case "whatweb": toolFn = hackerTools.whatweb; break;
          case "tlsx": toolFn = hackerTools.tlsx; break;
          case "katana": toolFn = hackerTools.katana; break;
          default:
            return `[mission] Unknown passive tool: ${tool}. Only passive recon tools allowed.`;
        }

        // Offline/test mode: never run external recon tools when network is skipped.
        // Without this, execute-next would block on real binaries (e.g. subfinder)
        // and ignore the caller's explicit request to stay off the network.
        if (process.env.PHANTOM_RECON_SKIP_NETWORK === "1") {
          const offlineResult = "(offline: network skipped via PHANTOM_RECON_SKIP_NETWORK)";
          saveResult(missionArg, resultKey, { target, tool, result: offlineResult, phase, technique, timestamp: new Date().toISOString() });
          appendActivity(missionArg, { action: "tool_skipped", phase, tool, target, technique, reason: "PHANTOM_RECON_SKIP_NETWORK" });
          return `[mission] ⏭ Skipped ${tool} on ${target} (phase ${phase}, ${technique}) — network skipped (PHANTOM_RECON_SKIP_NETWORK=1). Action recorded.`;
        }
        ...
        try {
          ...
          if (tool === "dnsx") {
            // dnsx expects subdomains as space-separated string
            // We need to get subdomains from previous subfinder results
            const existingSubs = loadExistingResult(missionArg, getResultKey(1, "subfinder", target));
            if (existingSubs && existingSubs.result) {
              const subs = existingSubs.result.split("\n").filter(l => l.trim() && !l.startsWith("[")).slice(0, 50);
              if (subs.length > 0) {
                result = await toolFn(subs.join(" "));
              } else {
                result = "(no subdomains to resolve)";
              }
            } else {
              result = "(no prior subfinder results for dnsx)";
            }
          } else {
            result = await toolFn(target);
          }

          // Persist the structured result through existing recon storage
          saveResult(missionArg, resultKey, { target, tool, result, phase, technique, timestamp: new Date().toISOString() });
          appendActivity(missionArg, { action: "tool_complete", phase, tool, target, technique });

          return `[mission] ✅ Executed ${tool} on ${target} (phase ${phase}, ${technique})\nResult saved.\n${result.substring(0, 500)}${result.length > 500 ? "..." : ""}`;
        } catch (e) {
          // Tool failure recovery - save error and continue
          saveError(missionArg, phase, tool, target, e);
          appendActivity(missionArg, { action: "tool_error", phase, tool, target, error: String(e), technique });
          return `[mission] Tool ${tool} failed on ${target}: ${e.message}. Error recorded, mission can continue.`;
        }
      }

      return `[mission] Unknown command: ${cmd}. Use @mission help`;
    } catch (e) {
      return `[mission Error] ${e.message}`;
    }
  },

  // ── MISSION RECON — autonomous mission execution ──
  mission_recon: async (input) => {
    const { startRecon, pauseRecon, resumeRecon, cancelMission, stopRecon, getExecutionState, getExecutionSummary, generateReconSummary } = await import("./recon.mjs");
    const parts = (input || "").trim().split(/\s+/);
    const cmd = parts[0]?.toLowerCase();
    const missionId = parts[1];
    
    if (!cmd || cmd === "help") {
      return `🎯 Mission Recon Execution
Usage: @mission_recon <command> <mission_id>

Commands:
  start <mission_id>   — Start autonomous recon (phases 1-3)
  pause <mission_id>   — Pause running recon
  resume <mission_id>  — Resume paused/failed recon
  stop <mission_id>    — Gracefully stop recon (allows resume)
  cancel <mission_id>  — Cancel mission permanently
  status <mission_id>  — Show execution state
  summary <mission_id> — Generate final report

Examples:
  @mission_recon start mission-abc123
  @mission_recon pause mission-abc123
  @mission_recon summary mission-abc123`;
    }
    
    if (!missionId) return `[mission_recon] Usage: @mission_recon <command> <mission_id>`;
    
    if (cmd === "start") {
      try {
        const findings = await startRecon(missionId);
        const summary = generateReconSummary(missionId);
        return `✅ Reconnaissance started for ${missionId}\nCompleted ${findings.length} findings across 3 phases.\n${summary}`;
      } catch (e) { return `[mission_recon] ${e.message}`; }
    }
    
    if (cmd === "pause") {
      try { return `⏸️ ${pauseRecon(missionId).message || "Paused"} — State: ${pauseRecon(missionId).state}`; }
      catch (e) { return `[mission_recon] ${e.message}`; }
    }
    
    if (cmd === "resume") {
      try { const findings = await resumeRecon(missionId); return `▶️ Resumed ${missionId} — ${findings.length} findings`; }
      catch (e) { return `[mission_recon] ${e.message}`; }
    }
    
    if (cmd === "stop") {
      try { const result = stopRecon(missionId); return result.success ? `🛑 ${result.message} — State: ${result.state}` : `ℹ️ ${result.message}`; }
      catch (e) { return `[mission_recon] ${e.message}`; }
    }
    
    if (cmd === "cancel") {
      try { const result = cancelMission(missionId); return `❌ Mission ${missionId} cancelled — State: ${result.state}`; }
      catch (e) { return `[mission_recon] ${e.message}`; }
    }
    
    if (cmd === "status") {
      try { const state = getExecutionState(missionId); return state ? `📊 ${missionId}: ${state}` : `[mission_recon] No execution state`; }
      catch (e) { return `[mission_recon] ${e.message}`; }
    }
    
    if (cmd === "summary") {
      try { const summary = generateReconSummary(missionId); return summary || `[mission_recon] Mission not found`; }
      catch (e) { return `[mission_recon] ${e.message}`; }
    }
    
    return `[mission_recon] Unknown command: ${cmd}. Use @mission_recon help`;
  },

  // ── RECON ANALYSIS — analyze persisted recon results ──
  recon_analyze: async (input) => {
    try {
      const { analyzeMission, analyzeAndSummarize, loadLearning, loadSummary, listLearnings, getLearningStats } = await import("./recon-analysis.mjs");
      const parts = (input || "").trim().split(/\s+/);
      const cmd = parts[0]?.toLowerCase();
      const missionId = parts[1];

      if (!cmd || cmd === "help") {
        return `📊 Recon Analysis\nUsage: @recon_analyze <command> [mission_id]\n\nCommands:\n  run <mission_id>     — Analyze persisted recon results & generate learning\n  summary <mission_id> — Show summary of observations\n  learning <mission_id> — Show extracted learning\n  list                 — List all analyzed missions\n  stats                — Aggregate learning stats across missions\n\nExample:\n  @recon_analyze run mission-abc123`;
      }

      if (cmd === "list") {
        const missions = listLearnings();
        if (missions.length === 0) return "No analyzed missions yet";
        return missions.map(m => `  ${m}`).join("\n");
      }

      if (cmd === "stats") {
        const stats = getLearningStats();
        return `📈 Learning Stats\nMissions analyzed: ${stats.totalMissions}\nSuccessful techniques: ${stats.successfulTechniques.join(", ") || "none"}\nFailed tools: ${stats.failedTools.join(", ") || "none"}\nDiscovery methods: ${stats.discoveryMethods.join(", ") || "none"}`;
      }

      if (!missionId) {
        return `[recon_analyze] Usage: @recon_analyze <command> <mission_id>`;
      }

      if (cmd === "run") {
        const { summary } = await analyzeMission(missionId);
        return analyzeAndSummarize(missionId);
      }

      if (cmd === "summary") {
        const summary = loadSummary(missionId);
        if (!summary) return `[recon_analyze] No summary for ${missionId}. Run 'run' first.`;
        return JSON.stringify(summary, null, 2);
      }

      if (cmd === "learning") {
        const learning = loadLearning(missionId);
        if (!learning) return `[recon_analyze] No learning for ${missionId}. Run 'run' first.`;
        return JSON.stringify(learning, null, 2);
      }

      return `[recon_analyze] Unknown command: ${cmd}. Use @recon_analyze help`;
    } catch (e) {
      return `[recon_analyze Error] ${e.message}`;
    }
  },

  // ── RECON INTELLIGENCE — read-only mission intelligence ──
  recon_intelligence: async (input) => {
    try {
      const { getMissionIntelligence, formatIntelligenceReport } = await import("./recon-intelligence.mjs");
      const parts = (input || "").trim().split(/\s+/);
      const cmd = parts[0]?.toLowerCase();
      const missionId = parts[1];

      if (!cmd || cmd === "help") {
        return `🧠 Recon Intelligence\nUsage: @recon_intelligence <command> [mission_id]\n\nCommands:\n  report <mission_id>   — Full intelligence report (default)\n  summary <mission_id>  — JSON observation summary\n  tech <mission_id>     — Technology summary\n  http <mission_id>     — HTTP services summary\n  dns <mission_id>      — DNS records summary\n  security <mission_id> — Security metadata summary\n  learned <mission_id>  — Learned techniques/failures\n  actions <mission_id>  — Recommended next passive actions\n  global                — Global learning stats across all missions\n\nExample:\n  @recon_intelligence report mission-abc123`;
      }

      if (cmd === "global") {
        const { getGlobalLearningStats } = await import("./recon-intelligence.mjs");
        const stats = getGlobalLearningStats();
        return `📈 Global Learning Stats\nMissions analyzed: ${stats.missionsAnalyzed}\nSuccessful techniques: ${Object.entries(stats.successfulTechniques).map(([k,v]) => `${k}:${v}`).join(", ") || "none"}\nTool failure rates: ${Object.entries(stats.toolFailures).map(([k,v]) => `${k}:${v}`).join(", ") || "none"}\nAvg duration: ${stats.averageDuration ? Math.round(stats.averageDuration) + "ms" : "n/a"}`;
      }

      if (!missionId) {
        return `[recon_intelligence] Usage: @recon_intelligence <command> <mission_id>`;
      }

      if (cmd === "summary") {
        const { getMissionObservationSummary } = await import("./recon-intelligence.mjs");
        const summary = getMissionObservationSummary(missionId);
        return JSON.stringify(summary, null, 2);
      }

      if (cmd === "tech") {
        const { getTechnologySummary } = await import("./recon-intelligence.mjs");
        return JSON.stringify(getTechnologySummary(missionId), null, 2);
      }

      if (cmd === "http") {
        const { getHttpSummary } = await import("./recon-intelligence.mjs");
        return JSON.stringify(getHttpSummary(missionId), null, 2);
      }

      if (cmd === "dns") {
        const { getDnsSummary } = await import("./recon-intelligence.mjs");
        return JSON.stringify(getDnsSummary(missionId), null, 2);
      }

      if (cmd === "security") {
        const { getSecurityMetadataSummary } = await import("./recon-intelligence.mjs");
        return JSON.stringify(getSecurityMetadataSummary(missionId), null, 2);
      }

      if (cmd === "learned") {
        const { getLearnedSummary } = await import("./recon-intelligence.mjs");
        return JSON.stringify(getLearnedSummary(missionId), null, 2);
      }

      if (cmd === "actions") {
        const { getRecommendedNextActions } = await import("./recon-intelligence.mjs");
        return JSON.stringify(getRecommendedNextActions(missionId), null, 2);
      }

      if (cmd === "report") {
        const { formatIntelligenceReport } = await import("./recon-intelligence.mjs");
        return formatIntelligenceReport(missionId);
      }

      return `[recon_intelligence] Unknown command: ${cmd}. Use @recon_intelligence help`;
    } catch (e) {
      return `[recon_intelligence Error] ${e.message}`;
    }
  },

  // ── MISSION PLANNER — deterministic passive next-action planner ──
  mission_next: async (input) => {
    try {
      const { getMissionPlan, getNextAction, getAllPlannedActions } = await import("./recon-planner.mjs");
      const parts = (input || "").trim().split(/\s+/);
      const cmd = parts[0]?.toLowerCase();
      const missionId = parts[1];

      if (!cmd || cmd === "help") {
        return `🎯 Mission Next-Action Planner\nUsage: @mission_next <command> [mission_id]\n\nCommands:\n  plan <mission_id>   — Full ranked action plan (JSON)\n  next <mission_id>   — Single highest-priority action\n  actions <mission_id> — All planned actions (alias for plan)\n\nExample:\n  @mission_next next mission-abc123`;
      }

      if (!missionId) {
        return `[mission_next] Usage: @mission_next <command> <mission_id>`;
      }

      if (cmd === "plan" || cmd === "actions") {
        const plan = getAllPlannedActions(missionId);
        return JSON.stringify(plan, null, 2);
      }

      if (cmd === "next") {
        const next = getNextAction(missionId);
        return JSON.stringify(next, null, 2);
      }

      return `[mission_next] Unknown command: ${cmd}. Use @mission_next help`;
    } catch (e) {
      return `[mission_next Error] ${e.message}`;
    }
  },
