...
export const hackerTools = {
  shell: async (cmd) => {
    ...
    const dangerous = ["rm ", "dd ", ">", "|", "mkfs", "fdisk", "format", ":(){", "chmod 777", "chown", "sudo ", "wget ", "curl ", "bash -", "sh -c"];
  ...
  web_search: async (query) => {
    ...
    try {
      ...
      if (ddg) {
        ...
        if (ddg.Infobox?.content) ddg.Infobox.content.forEach(c => lines.push(`  ${c.label}: ${c.value}`));
        if (ddg.Results?.length) ddg.Results.slice(0, 3).forEach(res => lines.push(`🔗 ${res.Text || res.FirstURL}`));
        if (ddg.RelatedTopics?.length) {
          const topics = ddg.RelatedTopics.flatMap(t => t.Topics || [t]).slice(0, 6);
          topics.forEach(t => { if (t.Text) lines.push(`  • ${t.Text}`); });
        }
        source = "duckduckgo";
      }

      // 2. If DDG gave nothing useful, search Wikipedia
      if (!lines.length || lines.length < 3) {
        ...
        if (wikiResults.length) {
          ...
          wikiResults.slice(0, 5).forEach(r => {
      ...
      // 3. If query looks CVE-ish, mention cve_search exists
      if (/cve|cwe|vulnerability|exploit/i.test(q)) {
        lines.push(`\n🔧 Tip: use @cve_search|${q.replace(/^what (is|are|about)\s+/i, "").trim()} for CVE details`);
      }
  ...
  youtube_transcript: async (input) => {
    ...
    try {
      ...
      for (const t of tokens) {
        ...
        else if (t === "--lang") { /* skip, next token is lang */ }
        else if (langParts.length % 2 === 0 && tokens[tokens.indexOf(t) - 1] === "--lang") langParts.push(t);
      ...
      let python = "/tmp/yt_venv/bin/python3";
      try {
        const { execSync } = await import("child_process");
        execSync(`test -f ${python}`, { timeout: 3000 });
      } catch {
        python = "python3";
      }

      const langParam = langParts.length ? `--language ${langParts.join(",")}` : "";

      const script = `
from youtube_transcript_api import YouTubeTranscriptApi
api = YouTubeTranscriptApi()
t = api.fetch("${videoId.replace(/"/g, '\\"')}", languages=${JSON.stringify(langParts.length ? langParts : ["en"])})
segs = list(t)
result = []
for s in segs:
    line = f"[{s.start:.1f}s] {s.text}" if ${timestamps ? "True" : "False"} else s.text
    result.append(line)
...
`;
      const { execSync } = await import("child_process");
      const out = execSync(python, { input: script, encoding: "utf-8", timeout: 30000, maxBuffer: 1024 * 1024 });
      const lines = out.trim().split("\n");
      const segLine = lines.find(l => l.startsWith("SEGS:"));
      const segCount = segLine ? parseInt(segLine.split(":")[1]) : 0;
      const startIdx = lines.indexOf("---") + 1;
      const transcript = lines.slice(startIdx).join("\n").trim();

      if (!transcript) return `[youtube_transcript] No transcript for video "${videoId}"`;

      const title = videoId; // Would need yt-dlp for better title extraction

      if (textOnly) return `${transcript.slice(0, 8000)}`;
      return `🎬 YouTube Transcript: "${title}"\\n  Segments: ${segCount || "?"}\\n  Length: ${transcript.length} chars\\n\\n${transcript.slice(0, 5000)}${transcript.length > 5000 ? "\\n...[truncated, " + (transcript.length - 5000) + " chars remain]" : ""}`;
    } catch (e) {
      if (e.message && e.message.includes("TranscriptsDisabled"))
        return `[youtube_transcript] Transcript disabled for this video`;
