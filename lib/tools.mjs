...
export const hackerTools = {
  ...
  install: async (tool) => {
    ...
    const pmap = {
      ...
      trufflehog:"trufflehog", arjun:"arjun", gospider:"gospider", whatweb:"whatweb",
      wafw00f:"wafw00f", masscan:"masscan", nikto:"nikto", hydra:"hydra", interactsh:"interactsh-client",
    };
  ...
  Examples:
    ...
    @nmap|192.168.1.1 -p 1-1000                        Port range
    @nmap|scanme.org -sV -sC -O                        Version+scripts+OS
    @nmap|scanme.org -p- --min-rate 1000               All ports fast
    @nmap|-sn 192.168.1.0/24                           Ping sweep
    @nmap|scanme.org -sS -sU -p 1-1000                 TCP SYN + UDP
  Pro tip: combine with scope — @scope check target before scanning`;
      try { execFileSync("which",["nmap"],{encoding:"utf-8",timeout:5000}); } catch { return `[nmap] NOT INSTALLED — install: apt install nmap (Linux), brew install nmap (macOS)`; }
      const parts = input.trim().split(/\s+/);
  ...
  Examples:
    @sqlmap|-u https://target.com/page?id=1                   Basic test
    @sqlmap|-u https://target.com/page?id=1 --dbs             Enumerate databases
    @sqlmap|-u https://target.com/page?id=1 -D dbname --tables Get tables
    @sqlmap|-u https://target.com/page?id=1 --os-shell        OS shell (if DBA)
    @sqlmap|-u https://target.com/page?id=1 --batch --random-agent  Auto mode
    @sqlmap|-r request.txt                                    From saved request`;
      try { execFileSync("which",["sqlmap"],{encoding:"utf-8",timeout:5000}); } catch { return `[sqlmap] NOT INSTALLED — install: apt install sqlmap (Kali/Parrot), brew install sqlmap (macOS)`; }
  ...
  Identifies web technologies: CMS, frameworks, JS libs, servers, etc.
  Examples:
    @whatweb|https://example.com                       Basic fingerprint
    @whatweb|https://example.com --aggression 3        Aggressive probing
    @whatweb|targets.txt --input-file                  Batch from file
    @whatweb|https://example.com --list-plugins        List all detection plugins`;
      try { execFileSync("which",["whatweb"],{encoding:"utf-8",timeout:5000}); } catch { return `[whatweb] NOT INSTALLED — install: apt install whatweb (Kali), brew install whatweb (macOS)`; }
      const parts = input.trim().split(/\s+/);
      const target = parts[0]; const extra = parts.slice(1);
  ...
  Detects Web Application Firewalls (Cloudflare, Cloudfront, Akamai, etc.)
  Examples:
    @wafw00f|https://example.com                       Detect WAF
    @wafw00f|https://example.com -a                    Find all WAFs
    @wafw00f|targets.txt                               Batch from file`;
      try { execFileSync("which",["wafw00f"],{encoding:"utf-8",timeout:5000}); } catch { return `[wafw00f] NOT INSTALLED — install: apt install wafw00f (Kali), pip install wafw00f`; }
      const parts = input.trim().split(/\s+/);
      const target = parts[0]; const extra = parts.slice(1);
      const out = execFileSync("wafw00f", [target,...extra], {encoding:"utf-8",timeout:120000,maxBuffer:1024*1024});
      return out.trim() || `[wafw00f] No WAF detected for ${target}`;
  ...
  Scans git repos, filesystems, S3 for leaked secrets/credentials.
  Examples:
    @trufflehog|https://github.com/org/repo.git         Scan git repo
    @trufflehog|filesystem /path/to/dir                 Scan filesystem
    @trufflehog|s3://bucket-name                        Scan S3 bucket
    @trufflehog|gitlab https://gitlab.com/org/repo      Scan GitLab`;
      try { execFileSync("which",["trufflehog"],{encoding:"utf-8",timeout:5000}); } catch { return `[trufflehog] NOT INSTALLED — install: go install github.com/trufflesecurity/trufflehog/v3@latest`; }
      const parts = input.trim().split(/\s+/);
      const sourceType = parts[0]; const target = parts[1]; const extra = parts.slice(2);
      const args = sourceType === "filesystem" ? ["filesystem",target,...extra,"--no-update"]
                 : sourceType.startsWith("s3") ? ["s3","--bucket",target||sourceType,...extra,"--no-update"]
                 : ["git",target||sourceType,...extra,"--no-update"];
      ...
      const verified = lines.filter(l => l.includes("Verified") || l.includes("true")).length;
      const result = [`🔐 TruffleHog: ${target||sourceType}`, `Findings: ${lines.length}${verified?` (${verified} verified)`:""}`];
  ...
  hydra: async (input) => {
    try {
      const { execFileSync } = await import("child_process");
      if (!input || !input.trim()) return `[hydra] Usage: @hydra|<target>|<protocol>|<user>|<passwords> [options]
  Online password brute-force. Protocols: ssh, ftp, http-post, mysql, rdp, smb, pop3, smtp, vnc, telnet, redis, mongodb
  Examples:
    @hydra|192.168.1.1|ssh|root|admin,toor,1234
    @hydra|target.com|http-post|admin|/login:user=^USER^&pass=^PASS^:F=incorrect|passwords.txt
    @hydra|target.com|ftp|anonymous|pass@test.com
  Use @scope check before brute-forcing — rate limiting may trigger WAFs`;
      try { execFileSync("which",["hydra"],{encoding:"utf-8",timeout:5000}); } catch { return `[hydra] NOT INSTALLED — install: apt install hydra (Kali), brew install hydra (macOS)`; }
      const parts = input.trim().split("|").map(s=>s.trim());
      if (parts.length < 3) return `[hydra] Need at least: target|protocol|user|passwords`;
      const target = parts[0]; const proto = parts[1]; const user = parts[2]; const pass = parts[3]||"";
      const extra = parts.slice(4);
      const passFile = pass.includes("/")||pass.includes(".") ? pass : `/tmp/hydra_pass_${Date.now()}.txt`;
      if (!pass.includes("/")&&!pass.includes(".")) (await import("fs")).writeFileSync(passFile, pass.replace(/,/g,"\n"));
      const args = ["-l",user,"-P",pass.includes("/")||pass.includes(".")?pass:passFile,`${proto}://${target}`,...extra,"-o","/dev/stdout","-t","4","-w","10","-vV"];
      const out = execFileSync("hydra", args, {encoding:"utf-8",timeout:300000,maxBuffer:1024*1024});
      try { if (!pass.includes("/")&&!pass.includes(".")) (await import("fs")).unlinkSync(passFile); } catch {}
      const lines = out.trim().split("\n").filter(Boolean);
      const found = lines.filter(l => l.includes("password:") || l.includes("login:") || l.includes("[SUCCESS]"));
      return found.length ? [`🔑 Hydra: ${target} (${proto})`,...found.map(l=>`  ${l}`)].join("\n") : `[hydra] No credentials found for ${user}@${target}`;
  ...
  Ultra-fast port scanner (can scan entire internet). Requires sudo/root.
  Examples:
    @masscan|192.168.1.0/24 -p 80,443,22,3389           Scan subnet
    @masscan|10.0.0.0/8 -p 443 --rate 1000              Large range
    @masscan|scanme.org -p 1-65535 --rate 1000          All ports
    @masscan|targets.txt -p 80,443 --rate 500            From file`;
      try { execFileSync("which",["masscan"],{encoding:"utf-8",timeout:5000}); } catch { return `[masscan] NOT INSTALLED — install: apt install masscan (Kali), brew install masscan (macOS)`; }
      const parts = input.trim().split(/\s+/);
      const target = parts[0]; const extra = parts.slice(1);
      const out = execFileSync("masscan", [target,...extra,"-oG","/dev/stdout","--rate",extra.includes("--rate")?"":"1000"], {encoding:"utf-8",timeout:300000,maxBuffer:2*1024*1024});
      const lines = out.trim().split("\n").filter(l => l.startsWith("Host:"));
      if (!lines.length) return `[masscan] No open ports found on ${target}`;
      return [`⚡ Masscan: ${target}`, `Open ports: ${lines.length}`,...lines.slice(0,100).map(l=>`  ${l}`)].join("\n");
  ...
  Web server scanner — finds outdated files, config issues, known vulns.
  Examples:
    @nikto|-h https://example.com                       Basic scan
    @nikto|-h https://example.com -ssl -port 443         Force SSL
    @nikto|-h https://example.com -Tuning 123456        Tuning by category
    @nikto|-h targets.txt                                Batch from file`;
      try { execFileSync("which",["nikto"],{encoding:"utf-8",timeout:5000}); } catch { return `[nikto] NOT INSTALLED — install: apt install nikto (Kali), brew install nikto (macOS)`; }
      const parts = input.trim().split(/\s+/);
      const target = parts[0]; const extra = parts.slice(1);
      const out = execFileSync("nikto", [...parts,...extra], {encoding:"utf-8",timeout:300000,maxBuffer:2*1024*1024});
      const lines = out.trim().split("\n").filter(Boolean);
      const findings = lines.filter(l => l.includes("+") && !l.includes("-------") && !l.includes("Target IP"));
      if (!findings.length) return `[nikto] 0 findings for ${target||parts[1]||""}`;
      return [`🔎 Nikto: ${parts[1]||target||""}`, `Findings: ${findings.length}`,...findings.slice(0,80).map(l=>`  ${l}`),...(findings.length>80?[`  ... and ${findings.length-80} more`]:[])].join("\n");
  ...
  Examples:
    @arjun|-u https://example.com/api/endpoint           Find params
    @arjun|-u https://example.com/page.php --get         Force GET
    @arjun|-u https://example.com/login --post           Force POST
    @arjun|-u https://example.com --headers              Discover header params
    @arjun|targets.txt                                   Batch mode`;
      try { execFileSync("which",["arjun"],{encoding:"utf-8",timeout:5000}); } catch { return `[arjun] NOT INSTALLED — install: pip install arjun`; }
      const parts = input.trim().split(/\s+/);
  ...
  Web spider/crawler that discovers links, forms, scripts, S3 buckets.
  Examples:
    @gospider|-s https://example.com                     Basic spider
    @gospider|-s https://example.com -d 2                Depth 2
    @gospider|-s https://example.com -c 20               Concurrency 20
    @gospider|-S targets.txt                             Batch from file
    @gospider|-s https://example.com --js                Extract JS endpoints`;
      try { execFileSync("which",["gospider"],{encoding:"utf-8",timeout:5000}); } catch { return `[gospider] NOT INSTALLED — install: go install github.com/jaeles-project/gospider@latest`; }
      const parts = input.trim().split(/\s+/);
      ...
      const result = [`🕷️ Gospider: ${parts[1]||""}`, `URLs: ${urls.length}`, `Events: ${lines.length}`];
  ...
  Enumerates cloud resources: S3 buckets, Azure Blobs, GCP buckets, etc.
  Examples:
    @cloud_enum|targetname                              Enum across all providers
    @cloud_enum|targetname -k keywords.txt              Custom keyword list
    @cloud_enum|targetname --aws                        AWS only
    @cloud_enum|targetname --azure --gcp                Azure + GCP only`;
      try { execFileSync("which",["cloud_enum"],{encoding:"utf-8",timeout:5000}); } catch { return `[cloud_enum] NOT INSTALLED — install: pip install cloud_enum`; }
      const parts = input.trim().split(/\s+/);
      const keyword = parts[0]; const extra = parts.slice(1);
      const out = execFileSync("cloud_enum", ["-k",keyword,...extra,"-q"], {encoding:"utf-8",timeout:180000,maxBuffer:2*1024*1024});
      const lines = out.trim().split("\n").filter(Boolean);
      const found = lines.filter(l => l.includes("found") || l.includes("open") || l.includes("public") || l.includes("http"));
      if (!found.length) return `[cloud_enum] No cloud resources found for "${keyword}"`;
      return [`☁️ Cloud Enum: ${keyword}`, `Found: ${found.length}`,...found.slice(0,60).map(l=>`  ${l}`),...(found.length>60?[`  ... and ${found.length-60} more`]:[])].join("\n");
  ...
  Send notifications to Slack, Telegram, Discord, Pushover, etc.
  Requires provider config in ~/.config/notify/provider-config.yaml
  Examples:
    @notify|Scan complete — found 3 subdomains         Send message
    @notify|Critical CVE found in Apache               Alert all channels
    @notify|"Results:\n$(cat findings.txt)"            Pipe content
  Setup: https://github.com/projectdiscovery/notify`;
      try { execFileSync("which",["notify"],{encoding:"utf-8",timeout:5000}); } catch { return `[notify] NOT INSTALLED — install: go install github.com/projectdiscovery/notify/cmd/notify@latest`; }
      const out = execFileSync("notify", ["-silent","-data",input], {encoding:"utf-8",timeout:30000,maxBuffer:1024*1024});
      return `✅ Notification sent (${input.slice(0,50)}${input.length>50?"...":""})`;
  ...
  Out-of-band interaction/request bin for blind vuln detection.
  Actions: start (get URL), poll (check interactions), stop
  Examples:
    @interactsh|start                                   Get unique callback URL
    @interactsh|start -n 5                              Get 5 URLs
    @interactsh|poll https://oastify.com/1234           Check for callbacks
    @interactsh|stop                                    Stop interactions
  Use the URL in payloads: <url>=<payload> for SSRF, blind XSS, etc.`;
      try { execFileSync("which",["interactsh-client"],{encoding:"utf-8",timeout:5000}); } catch { return `[interactsh] NOT INSTALLED — install: go install github.com/projectdiscovery/interactsh/cmd/interactsh-client@latest`; }
      const parts = input.trim().split(/\s+/);
      ...
      if (action === "start") {
        ...
        return `📡 Interactsh: ${out.trim().split("\n")[0]||out.trim()}\nUse this URL in SSRF/blind XSS payloads`;
        ...
        const lines = out.trim().split("\n").filter(Boolean);
        return lines.length ? [`📡 Interactions (${lines.length}):`,...lines.slice(0,30).map(l=>`  ${l}`)].join("\n") : `[interactsh] No interactions for ${parts[1]}`;
      } else if (action === "stop") {
        return `[interactsh] Session stopped`;
      }
      return `[interactsh] Usage: start | poll <url> | stop`;
