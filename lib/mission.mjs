...
export function isInScope(target, mission) {
  ...
  for (const scopeEntry of mission.scope.inScope) {
...
export async function fetchProgramAndScope(handle) {
  ...
  // The result has literal \n (backslash-n) as line separators between scope entries,
  // and actual newlines within multi-line descriptions.
  // First split by literal \n to get the logical lines.
  const lines = result.split("\\n");
  let current = "in";
  
  for (const line of lines) {
    const trimmed = line.trim();
    // Check for section headers on the trimmed line
    if (trimmed.includes("IN SCOPE")) current = "in";
    else if (trimmed.includes("OUT OF SCOPE")) current = "out";
    // Check for scope entries on the raw line (which preserves leading spaces)
    else if (line.startsWith("  ") && trimmed.includes("(")) {
      // Match: "identifier (TYPE) — description" (trimmed has no leading spaces)
