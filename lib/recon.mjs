...
async function startRecon(missionId) {
  ...
  const SKIP_NET = process.env.PHANTOM_RECON_SKIP_NETWORK === "1";
  ...
  // Idle watchdog: only trips when NO progress happens for a while (a stall/hang).
  // Long-but-active recon keeps beating and is never cut off by a wall-clock limit.
  const IDLE_MS = parseInt(process.env.PHANTOM_IDLE_TIMEOUT_MS || "120000");
  let hung = false;
  const watchdog = createWatchdog({
    ...
    onIdle: (err) => {
      hung = true;
      appendActivity(missionId, { action: "idle_timeout", idleMs: IDLE_MS, beats: err.beats });
      try { setExecutionState(missionId, EXEC_STATES.FAILED); } catch {}
      try { updateMissionStatus(missionId, "failed"); } catch {}
    },
  });
  appendActivity(missionId, { action: "watchdog_armed", idleMs: IDLE_MS });
  
  // Phase 1: Passive recon
  const findings = [];
  const MAX_TARGETS = parseInt(process.env.PHANTOM_RECON_MAX_TARGETS || "50");
  let inScopeTargets = mission.scope.inScope.map(s => s.normalized);
  if (MAX_TARGETS > 0) inScopeTargets = inScopeTargets.slice(0, MAX_TARGETS);
  appendActivity(missionId, { action: "recon_targets", count: inScopeTargets.length, capped: MAX_TARGETS > 0 });
  
  for (const target of inScopeTargets) {
    if (hung || getExecutionState(missionId) !== EXEC_STATES.RECONNAISSANCE) break;
    watchdog.beat();
    
    try {
      if (SKIP_NET) {
        findings.push({ target, phase: 1, status: "success", tools: ["subfinder", "dnsx"], skipped: true });
        continue;
      }
      const { hackerTools } = await import("./tools.mjs");
      const { subfinder, dnsx, httpx } = hackerTools;
  ...
  if (!hung && getExecutionState(missionId) === EXEC_STATES.RECONNAISSANCE) {
    ...
    for (const target of inScopeTargets) {
      ...
      try {
        if (SKIP_NET) {
          findings.push({ target, phase: 2, status: "success", tools: ["httpx"], skipped: true });
          continue;
        }
        const { hackerTools } = await import("./tools.mjs");
        const httpx = hackerTools.httpx;
  ...
  watchdog.stop();
  if (hung) {
    appendActivity(missionId, { action: "recon_idle_aborted", idleMs: IDLE_MS });
  }
  return findings;
}
