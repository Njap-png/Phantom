...
import { status as vaultStatus } from "./vault.mjs";
...
export function populateEnv(existingEnv = {}) {
  ...
  let device = null;
  try {
    const cfgPath = resolve(homedir(), ".config", "phantom", "config.json");
    const cfg = JSON.parse(readFileSync(cfgPath, "utf-8"));
    environment = cfg.ENVIRONMENT || null;
    device = cfg.DEVICE || null;
  } catch {}

  Object.assign(existingEnv, { environment, device, tools, pkgMgr, shell, network, resources, availableTools: availableCount, totalChecked });
}
...
export function getEnvSummary(env) {
  ...
  if (env.device) lines.push(`Device: ${env.device}`);
  try { lines.push(`Vault: ${vaultStatus().location}`); } catch {}
