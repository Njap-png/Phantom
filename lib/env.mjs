...
function detectTools() {
  const all = [...RECON_BINS, ...EXPLOIT_BINS, ...DEV_BINS, ...NET_BINS, ...BROWSER_BINS];
  const tools = {};
  // Single shell call checks all ~38 binaries in parallel (~100ms vs 76s sequential)
  try {
    const script = all.map(t => `command -v ${t} 2>/dev/null && echo "BIN:${t}"`).join(";");
    const out = execFileSync("sh", ["-c", script], { timeout: 5000, stdio: ["pipe", "pipe", "pipe"] });
    const found = new Set(out.toString().trim().split("\n").filter(l => l.startsWith("BIN:")).map(l => l.slice(4)));
    for (const t of all) tools[t] = found.has(t);
  } catch(e) {
    for (const t of all) tools[t] = false;
  }
  return tools;
}
...
export function autoInstallCore(envTools) {
  ...
  try {
    execFileSync("apt", ["install", "-y", ...missing.map(t => t === "python3" ? "python3" : t)], { timeout: 120000, stdio: "pipe" });
...
export function populateEnv(existingEnv = {}) {
  ...
  // Auto-install core missing tools (curl, wget, git, jq, python3)
  const installed = autoInstallCore({ pkgMgr, tools, ...existingEnv });
  if (installed > 0) {
