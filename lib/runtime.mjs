...
import { resolveToolBin } from "./install-tools.mjs";

// Lazy import for evolve — avoid circular dep at module load time
let _evolve = null;
async function getEvolve() {
  if (!_evolve) _evolve = await import("./evolve.mjs");
  return _evolve;
}
...
// ── Async shell exec helper ──────────────────────────────
// Replaces execSync throughout tools.mjs so the event loop stays
// responsive during long-running commands (stdin keeps working).
export function execAsync(cmd, opts = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    import("child_process").then(({ exec }) => {
      exec(cmd, { encoding: "utf-8", timeout, maxBuffer }, (err, stdout, stderr) => {
        if (err) {
          err.stderr = (stderr || "").substring(0, 500);
          return reject(err);
        }
        resolve(stdout);
      });
...
export function runExternal(tool, args, opts = {}) {
  ...
  // Resolve binary path — prefer self-installed (~/.config/phantom/bin), then PATH.
  // A self-installed binary overrides a stale "not installed" cache entry.
  const resolvedBin = resolveToolBin(tool);
  if (!resolvedBin) {
  ...
  const out = execFileSync(resolvedBin, args, execOpts);
...
export async function runTool(tools, toolName, input, opts = {}) {
  ...
  const tui = globalThis.__phantomTUI;
  const timer = tui?.active ? null : setTimeout(() => process.stdout.write(`[*] ${toolName} running...\r`), 2000);
  try {
    ...
    if (timer) clearTimeout(timer);
    // Auto-heal: try to fix the error and retry once
    try {
      const evolve = await getEvolve();
      const fix = await evolve.analyzeError(toolName, e.message);
      if (fix && fix.fixed) {
        // Retry the tool after successful fix
        const retryResult = await fn(input);
        if (opts.json) {
          return JSON.stringify({ ok: true, tool: toolName, input, data: retryResult, length: retryResult.length, auto_healed: true, fix: fix.message });
        }
        return `[auto-heal] ${fix.message}\n${retryResult}`;
      }
    } catch {}
