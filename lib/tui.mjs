...
export class TUI {
  constructor(opts = {}) {
    this.active = false;
    this._headerHeight = 8;   // logo (6) + blank + separator
  ...
  _drawStatic() {
    // Fixed header at top
    process.stdout.write(`${CSI}H`); // home
    const logo = renderLogo({ tools: this._toolCount || 0 });
    const lines = logo.split("\n");
    for (let i = 0; i < 6; i++) {
      process.stdout.write(lines[i] || "");
      process.stdout.write(`${CSI}K`); // clear to end
      if (i < 5) process.stdout.write("\n");
    }
    // Separator
    process.stdout.write(`\n${c("dim")}${"─".repeat(this._cols)}${R}`);
