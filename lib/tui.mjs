...
export class TUI {
  ...
  enter() {
    ...
    // Calculate conversation region
    // Logo takes _logoHeight lines, separator takes 1
    this._convTop = this._logoHeight + 2; // row after separator (1-based)
    this._convBot = this._rows - 2;       // row before input bar separator
    this._cursorRow = this._convTop;

    // Draw initial empty conversation area + input bar
    // We fill with blank lines so the layout is correct
    const convRows = this._convBot - this._convTop;
    for (let i = 0; i < convRows; i++) {
      process.stdout.write("\n");
    }
    // Input bar separator + input line
    process.stdout.write(dim("\u2500".repeat(this._cols)) + "\n");
  ...
  log(text) {
    ...
    // Always write from the conversation cursor position
    process.stdout.write(CSI + this._cursorRow + ";1H");

    // If the text would overflow past the conversation area,
    // we use the terminal's natural scroll by writing newlines
    // at the bottom, which scrolls the logo + conversation up.
    const overflow = (this._cursorRow + lineCount - 1) - this._convBot;

    if (overflow > 0) {
      // Move to the very bottom of the conversation area
      process.stdout.write(CSI + this._convBot + ";1H");
      // Write text — it will extend past convBot. Since the cursor
      // is at the bottom of the screen, writing \n will scroll everything up.
      process.stdout.write(text + "\n");
      // Cursor is now at the line after convBot. The terminal has scrolled.
      // We need to figure out where we are now.
      // After scrolling, the logo+conversation moved up by overflow rows.
      // The cursor ends up at convBot (the terminal scrolled it into the region).
      this._cursorRow = this._convBot;
    } else {
      // Fits within the conversation area
      process.stdout.write(text + "\n");
  ...
  _fullRedraw() {
    ...
    // Fill remaining conversation rows with blank lines
