...
export class TUI {
  ...
  log(text) {
    ...
    if (this._cursorRow < this._rows - 2) {
      ...
      // At bottom: jump to last row, write \n to trigger terminal scroll.
      // This pushes ALL content up by 1 row — logo scrolls away naturally.
      process.stdout.write(CSI + this._rows + ";1H");
      process.stdout.write("\n");
      // After scroll, cursorRow still points at the conversation row
      // (just above the input bar). The old content has scrolled up.
      // Clear the stale input separator that scrolled into cursorRow.
      process.stdout.write(CSI + this._cursorRow + ";1H" + CSI + "2K");
    }
