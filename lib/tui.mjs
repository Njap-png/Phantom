...
// Scroll region protects the logo+separator area. Conversation scrolls within
// the scroll region; input bar is fixed at bottom outside the region.
...
export class TUI {
  ...
  enter() {
    ...
    // Scroll region: conversation area (after separator, before input bar)
    ...
    // Draw input bar at bottom (outside region)
    this._redrawInput();
  ...
  log(text) {
    ...
    if (this._cursorRow > this._convBot) {
      process.stdout.write("\n");   // \n within scroll region — rows shift up
  ...
  _redrawInput() {
    // Input separator at convBot + 1 (first row below scroll region)
    const sepRow = this._convBot + 1;
    process.stdout.write(CSI + sepRow + ";1H" + CSI + "2K");
    process.stdout.write(dim("─".repeat(this._cols)));
    // Input bar at sepRow + 1
    process.stdout.write(CSI + (sepRow + 1) + ";1H" + CSI + "2K");
    process.stdout.write(c("green") + "▸ " + R + this._inputText);
    // Cursor at input text end
    process.stdout.write(CSI + (sepRow + 1) + ";" + (4 + this._inputText.length) + "H");
  }
}
