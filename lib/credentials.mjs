...
export function askHidden(question) {
  const promptText = String(question ?? "");
...
async function ask(question) {
  const prompt = globalThis.__phantomPrompt;
...
export async function ensure(envVar, opts = {}) {
  ...
  const current = pending.get(envVar);
  ...
  pending.set(envVar, request);
  try {
    return await request;
  } finally {
    if (pending.get(envVar) === request) pending.delete(envVar);
  }
}
